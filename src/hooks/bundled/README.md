...
This directory contains hooks that ship with Kova. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
- **Managed hooks**: `~/.kova/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.kova/openclaw.json`:
...
2. Restart gateway: `pkill -9 -f 'openclaw.*gateway' && pnpm kova gateway`
3. Enable the hook: `kova hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.kova.ai/automation/hooks
