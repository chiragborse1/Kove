...
export function formatHooksList(report: HookStatusReport, opts: HooksListOptions): string {
  ...
  if (hooks.length === 0) {
    const message = opts.eligible
      ? `No eligible hooks found. Run \`${formatCliCommand("kova hooks list")}\` to see all hooks.`
      : "No hooks found.";
    return message;
  }
...
export function registerHooksCli(program: Command): void {
  ...
  hooks
    .command("install")
    .description("Deprecated: install a hook pack via `kova plugins install`")
    ...
    .action(async (raw: string, opts: { link?: boolean; pin?: boolean }) => {
      defaultRuntime.log(
        theme.warn("`kova hooks install` is deprecated; use `kova plugins install`."),
      );
  ...
  hooks
    .command("update")
    .description("Deprecated: update hook packs via `kova plugins update`")
    ...
    .action(async (id: string | undefined, opts: HooksUpdateOptions) => {
      defaultRuntime.log(
        theme.warn("`kova hooks update` is deprecated; use `kova plugins update`."),
      );
