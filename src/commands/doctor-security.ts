...
export async function noteSecurityWarnings(cfg: OpenClawConfig) {
  ...
  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      ...
      `  Check local policy with: ${formatCliCommand("kova approvals get --gateway")}`,
    );
  }
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("kova configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("kova config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("kova doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("kova config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('kova config set session.dmScope "per-channel-peer"') +
