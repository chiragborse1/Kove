...
describe("agentLogoUrl", () => {
  it("keeps base-mounted control UI logo paths absolute to the mount", () => {
    expect(agentLogoUrl("/ui")).toBe("/ui/assets/kova-logo.png");
    expect(agentLogoUrl("/apps/openclaw/")).toBe("/apps/openclaw/assets/kova-logo.png");
  });

  it("uses a route-relative fallback before basePath bootstrap finishes", () => {
    expect(agentLogoUrl("")).toBe("/assets/kova-logo.png");
  });
});
