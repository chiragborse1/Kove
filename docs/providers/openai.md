---
summary: "Use OpenAI via API keys or Codex subscription in Kova"
read_when:
  - You want to use OpenAI models in Kova
...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like Kova.
...
OpenAI API usage. Kova forwards both through the `openai/*` Responses path.
Kova intentionally suppresses the stale `openai/gpt-5.3-codex-spark` row,
because direct OpenAI API calls reject it in live traffic.

Kova does **not** expose `openai/gpt-5.3-codex-spark` on the direct OpenAI
API path. `pi-ai` still ships a built-in row for that model, but live OpenAI API
requests currently reject it. Spark is treated as Codex-only in Kova.
...
OpenAI's current Codex docs list `gpt-5.4` as the current Codex model. Kova
maps that to `openai-codex/gpt-5.4` for ChatGPT/Codex OAuth usage.

If your Codex account is entitled to Codex Spark, Kova also supports:

- `openai-codex/gpt-5.3-codex-spark`

Kova treats Codex Spark as Codex-only. It does not expose a direct
`openai/gpt-5.3-codex-spark` API-key path.

Kova also preserves `openai-codex/gpt-5.3-codex-spark` when `pi-ai`
...
Kova uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), Kova also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. Kova enables it by default for
...
Kova, set `agents.defaults.models["<provider>/<model>"].params.serviceTier`
...
Kova forwards `params.serviceTier` to both direct `openai/*` Responses
...
- if you route either provider through another base URL or proxy, Kova leaves `service_tier` untouched

### OpenAI fast mode

Kova exposes a shared fast-mode toggle for both `openai/*` and
`openai-codex/*` sessions:

- Chat/UI: `/fast status|on|off`
- Config: `agents.defaults.models["<provider>/<model>"].params.fastMode`

When fast mode is enabled, Kova maps it to OpenAI priority processing:
...
`baseUrl` on `api.openai.com`), Kova now auto-enables OpenAI server-side
