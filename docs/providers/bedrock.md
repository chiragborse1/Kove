---
summary: "Use Amazon Bedrock (Converse API) models with Kova"
read_when:
  - You want to use Amazon Bedrock models with Kova
...
Kova can use **Amazon Bedrock** models via pi‑ai’s **Bedrock Converse**
...
If AWS credentials are detected, Kova can automatically discover Bedrock
...
When running Kova on an EC2 instance with an IAM role attached, the AWS SDK
will automatically use the instance metadata service (IMDS) for authentication.
However, Kova's credential detection currently only checks for environment
...
- Kova surfaces the credential source in this order: `AWS_BEARER_TOKEN_BEDROCK`,
