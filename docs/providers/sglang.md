---
summary: "Run Kova with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run Kova against a local SGLang server
...
Kova can connect to SGLang using the `openai-completions` API.

Kova can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, Kova will query:
