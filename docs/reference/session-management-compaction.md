...
This document explains how Kova manages sessions end-to-end:
...
Kova is designed around a single **Gateway process** that owns session state.
...
Kova persists sessions in two layers:
...
Kova resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, Kova reports potential evictions but does not mutate the store/files.
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.kova/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
Kova intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (Kova consumes the events, but Pi decides when to compact).
...
Kova also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Kova bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Kova leaves it alone.
...
Kova supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- Kova strips/suppresses this in the delivery layer.

As of `2026.1.10`, Kova also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
Kova uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Kova’s
