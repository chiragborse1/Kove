...
<Steps>
  <Step title="Existing config detection">
    - If `~/.kova/openclaw.json` exists, choose **Keep / Modify / Reset**.
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.kova/credentials/oauth.json`; auth profiles live in `~/.kova/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.kova/credentials/oauth.json` (or `$OPENCLAW_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
