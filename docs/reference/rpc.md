...
Kova integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Kova owns lifecycle when `channels.signal.autoStart=true`.
...
- Kova spawns `imsg rpc` as a child process (legacy iMessage integration).
