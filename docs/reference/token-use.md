---
summary: "How Kova builds prompt context and reports token usage + costs"
read_when:
...
Kova tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Kova assembles its own system prompt on every run. It includes:
...
For images, Kova downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Kova session logs.
...
`cacheWrite`. If pricing is missing, Kova shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Kova can
...
Anthropic's 1M context window is currently beta-gated. Kova can inject the
...
Kova skips the `context-1m-*` beta header because Anthropic currently
