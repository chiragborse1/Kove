...
Kova uses Bonjour (mDNS / DNS‑SD) as a **LAN‑only convenience** to discover
...
- serve your chosen domain (example: `openclaw.internal.`) from `~/.kova/dns/<domain>.db`
...
- Set `gateway.bind: "tailnet"` in `~/.kova/openclaw.json`.
...
- `gateway.bind` in `~/.kova/openclaw.json` controls the Gateway bind mode.
