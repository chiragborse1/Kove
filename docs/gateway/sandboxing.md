...
Kova can run **tools inside sandbox backends** to reduce blast radius.
...
Use `backend: "ssh"` when you want Kova to sandbox `exec`, file tools, and media reads on
...
Use `backend: "openshell"` when you want Kova to sandbox tools in an
...
- `openclaw sandbox recreate` deletes the current runtime and lets Kova recreate it on next use
...
- `"none"` (default): tools see a sandbox workspace under `~/.kova/sandboxes`.
...
Kova mirrors eligible skills into the sandbox workspace (`.../skills`) so
