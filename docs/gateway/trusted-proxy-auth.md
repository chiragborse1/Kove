...
read_when:
  - Running Kova behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Kova
...
- You run Kova behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Kova checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Kova extracts the user identity from the configured header
...
- Kova can stay on loopback HTTP behind the proxy.
...
If Kova itself serves HTTPS directly (no TLS-terminating proxy), set:
...
3. Update Kova config with trusted-proxy auth
