---
summary: "Where Kova loads environment variables and the precedence order"
read_when:
...
Kova pulls environment variables from multiple sources. The rule is **never override existing values**.

## Precedence (highest → lowest)

1. **Process environment** (what the Gateway process already has from the parent shell/daemon).
2. **`.env` in the current working directory** (dotenv default; does not override).
3. **Global `.env`** at `~/.kova/.env` (aka `$OPENCLAW_STATE_DIR/.env`; does not override).
4. **Config `env` block** in `~/.kova/openclaw.json` (applied only if missing).
...
Kova supports two env-driven patterns:
...
| Variable               | Purpose                                                                                                                                                                          |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `OPENCLAW_HOME`        | Override the home directory used for all internal path resolution (`~/.kova/`, agent dirs, sessions, credentials). Useful when running Kova as a dedicated service user. |
| `OPENCLAW_STATE_DIR`   | Override the state directory (default `~/.openclaw`).                                                                                                                            |
| `OPENCLAW_CONFIG_PATH` | Override the config file path (default `~/.kova/openclaw.json`).                                                                                                             |
...
On Linux, Kova automatically detects nvm and applies the fix in the actual startup environment:
...
Export the variable before starting Kova:
