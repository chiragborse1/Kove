---
summary: "Run Kova Gateway 24/7 on an Azure Linux VM with durable state"
read_when:
  - You want Kova running 24/7 on Azure with Network Security Group hardening
  - You want a production-grade, always-on Kova Gateway on your own Azure Linux VM
  - You want secure administration with Azure Bastion SSH
title: "Azure"
---

# Kova on Azure Linux VM

This guide sets up an Azure Linux VM with the Azure CLI, applies Network Security Group (NSG) hardening, configures Azure Bastion for SSH access, and installs Kova.
...
- Install Kova with the installer script
- Verify the Gateway
...
<Steps>
  ...
  <Step title="Install Kova (in the VM shell)">
    ```bash
    curl -fsSL https://kova.ai/install.sh -o /tmp/install.sh
    bash /tmp/install.sh
    rm -f /tmp/install.sh
    ```

    The installer installs Node LTS and dependencies if not already present, installs Kova, and launches the onboarding wizard. See [Install](/install) for details.
  ...
  <Step title="Verify the Gateway">
    ...
    Most enterprise Azure teams already have GitHub Copilot licenses. If that is your case, we recommend choosing the GitHub Copilot provider in the Kova onboarding wizard. See [GitHub Copilot provider](/providers/github-copilot).
...
- **Deallocate the VM** when not in use (stops compute billing; disk charges remain). The Kova Gateway will not be reachable while the VM is deallocated — restart it when you need it live again:
