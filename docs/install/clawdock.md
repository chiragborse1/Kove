---
summary: "ClawDock shell helpers for Docker-based Kova installs"
read_when:
...
ClawDock is a small shell-helper layer for Docker-based Kova installs.
...
| `clawdock-cli <command>`  | Run Kova CLI commands in Docker           |
...
- `~/.kova/.env` for provider keys and bot tokens
- `~/.kova/openclaw.json` for behavior config
