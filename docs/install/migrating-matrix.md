---
summary: "How Kova upgrades the previous Matrix plugin in place, including encrypted-state recovery limits and manual recovery steps."
read_when:
...
When the gateway starts, and when you run [`openclaw doctor --fix`](/gateway/doctor), Kova tries to repair old Matrix state automatically.
Before any actionable Matrix migration step mutates on-disk state, Kova creates or reuses a focused recovery snapshot.

When you use `openclaw update`, the exact trigger depends on how Kova is installed:
...
Kova cannot automatically recover:
...
1. Update Kova and the Matrix plugin normally.
...
5. If Kova tells you a recovery key is needed, run:
...
3. If a backup decryption key is found, Kova writes it into the new recovery-key flow and marks room-key restore as pending.
4. On the next Matrix startup, Kova restores backed-up room keys into the new crypto store automatically.

If the old store reports room keys that were never backed up, Kova warns instead of pretending recovery succeeded.
...
- Meaning: Kova created a recovery archive before mutating Matrix state.
- What to do: keep the printed archive path until you confirm migration succeeded.

`Matrix migration snapshot reused before applying Matrix upgrades.`

- Meaning: Kova found an existing Matrix migration snapshot marker and reused that archive instead of creating a duplicate backup.
- What to do: keep the printed archive path until you confirm migration succeeded.

`Legacy Matrix state detected at ... but channels.matrix is not configured yet.`

- Meaning: old Matrix state exists, but Kova cannot map it to a current Matrix account because Matrix is not configured.
- What to do: configure `channels.matrix`, then rerun `openclaw doctor --fix` or restart the gateway.

`Legacy Matrix state detected at ... but the new account-scoped target could not be resolved yet (need homeserver, userId, and access token for channels.matrix...).`

- Meaning: Kova found old state, but it still cannot determine the exact current account/device root.
...
- Meaning: Kova found one shared flat Matrix store, but it refuses to guess which named Matrix account should receive it.
...
- Meaning: the new account-scoped location already has a sync or crypto store, so Kova did not overwrite it automatically.
...
- Meaning: Kova tried to move old Matrix state but the filesystem operation failed.
- What to do: inspect filesystem permissions and disk state, then rerun `openclaw doctor --fix`.

`Legacy Matrix encrypted state detected at ... but channels.matrix is not configured yet.`

- Meaning: Kova found an old encrypted Matrix store, but there is no current Matrix config to attach it to.
- What to do: configure `channels.matrix`, then rerun `openclaw doctor --fix` or restart the gateway.

`Legacy Matrix encrypted state detected at ... but the account-scoped target could not be resolved yet (need homeserver, userId, and access token for channels.matrix...).`

- Meaning: the encrypted store exists, but Kova cannot safely decide which current account/device it belongs to.
...
- Meaning: Kova found one shared flat legacy crypto store, but it refuses to guess which named Matrix account should receive it.
- What to do: set `channels.matrix.defaultAccount` to the intended account, then rerun `openclaw doctor --fix` or restart the gateway.
...
- Meaning: Kova refused to mutate Matrix state because it could not create the recovery snapshot first.
- What to do: resolve the backup error, then rerun `openclaw doctor --fix` or restart the gateway.

`Failed migrating legacy Matrix client storage: ...`

- Meaning: the Matrix client-side fallback found old flat storage, but the move failed. Kova now aborts that fallback instead of silently starting with a fresh store.
...
- Meaning: backup exists, but Kova could not recover the recovery key automatically.
- What to do: run `openclaw matrix verify backup restore --recovery-key "<your-recovery-key>"`.

`Failed inspecting legacy Matrix encrypted state for account "..." (...): ...`

- Meaning: Kova found the old encrypted store, but it could not inspect it safely enough to prepare recovery.
...
- Meaning: Kova detected a backup key conflict and refused to overwrite the current recovery-key file automatically.
...
- Meaning: Kova knows you should have a backup key, but it is not active on this device.
