...
Goal: Kova Gateway running on an exe.dev VM, reachable from your laptop via: `https://<vm-name>.exe.xyz`
...
Shelley, [exe.dev](https://exe.dev)'s agent, can install Kova instantly with our
...
Tip: keep this VM **stateful**. Kova stores state under `~/.kova/` and `~/.kova/workspace/`.
...
Run the Kova install script:

```bash
curl -fsSL https://kova.ai/install.sh | bash
```

## 4) Setup nginx to proxy Kova to port 8000
