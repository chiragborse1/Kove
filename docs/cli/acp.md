...
openclaw acp --url wss://gateway-host:18789 --token-file ~/.kova/gateway.token
...
openclaw acp client --server-args --url wss://gateway-host:18789 --token-file ~/.kova/gateway.token
...
If you want ACPX-backed sessions to see Kova plugin tools, enable the
...
Kova bot over ACP, use `acpx` with its built-in `openclaw` target.
...
acpx openclaw exec "Summarize the active Kova session state."
...
```json
{
  "agents": {
    "openclaw": {
      "command": "env OPENCLAW_HIDE_BANNER=1 OPENCLAW_SUPPRESS_NOTES=1 openclaw acp --url ws://127.0.0.1:18789 --token-file ~/.kova/gateway.token --session agent:main:main"
    }
  }
}
```
