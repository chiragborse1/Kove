---
summary: "macOS app flow for controlling a remote Kova gateway over SSH"
read_when:
...
- **Remote over SSH (default)**: Kova commands are executed on the remote host. The mac app opens an SSH connection with `-o BatchMode` plus your chosen identity/key and a local port-forward.
...
1. Install Node + pnpm and build/install the Kova CLI (`pnpm install && pnpm build && pnpm link --global`).
...
2. Under **Kova runs**, pick **Remote over SSH** and set:
