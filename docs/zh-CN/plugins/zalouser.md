---
read_when:
  - 你想在 Kova 中支持 Zalo Personal（非官方）
...
通过插件为 Kova 提供 Zalo Personal 支持，使用 `zca-cli` 自动化普通 Zalo 用户账户。
...
openclaw message send --channel zalouser --target <threadId> --message "Hello from Kova"
