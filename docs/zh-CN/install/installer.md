...
Kova 提供三个安装脚本，由 `kova.ai` 提供。

| 脚本                               | 平台                  | 功能                                                                          |
| ---------------------------------- | --------------------- | ----------------------------------------------------------------------------- |
| [`install.sh`](#installsh)         | macOS / Linux / WSL   | 如有需要则安装 Node，通过 npm（默认）或 git 安装 Kova，并可运行新手引导。 |
| [`install-cli.sh`](#install-clish) | macOS / Linux / WSL   | 将 Node + Kova 安装到本地前缀（`~/.openclaw`）中。无需 root。             |
| [`install.ps1`](#installps1)       | Windows（PowerShell） | 如有需要则安装 Node，通过 npm（默认）或 git 安装 Kova，并可运行新手引导。 |
...
<Tabs>
  <Tab title="install.sh">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://kova.ai/install.sh | bash
    ```

    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://kova.ai/install.sh | bash -s -- --help
    ```

  </Tab>
  <Tab title="install-cli.sh">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://kova.ai/install-cli.sh | bash
    ```

    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://kova.ai/install-cli.sh | bash -s -- --help
    ```
  ...
  <Tab title="install.ps1">
    ...
    ```powershell
    & ([scriptblock]::Create((iwr -useb https://kova.ai/install.ps1))) -Tag beta -NoOnboard -DryRun
    ```
...
<Steps>
  ...
  <Step title="默认确保使用 Node.js 24">
    检查 Node 版本，并在需要时安装 Node 24（macOS 上使用 Homebrew，Linux apt/dnf/yum 上使用 NodeSource 设置脚本）。为了兼容性，Kova 仍支持 Node 22 LTS，目前为 `22.16+`。
  </Step>
  <Step title="确保安装 Git">
    如果缺少 Git，则安装它。
  </Step>
...
如果在 Kova 检出目录中运行（`package.json` + `pnpm-workspace.yaml`），脚本会提供：
...
<Tabs>
  ...
  <Tab title="跳过新手引导">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://kova.ai/install.sh | bash -s -- --no-onboard
    ```
  </Tab>
  <Tab title="Git 安装">
    ```bash
  ...
  <Tab title="通过 npm 安装 GitHub main">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://kova.ai/install.sh | bash -s -- --version main
    ```
  </Tab>
  <Tab title="试运行">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://kova.ai/install.sh | bash -s -- --dry-run
    ```
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="自定义前缀 + 版本">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://kova.ai/install-cli.sh | bash -s -- --prefix /opt/openclaw --version latest
    ```
  </Tab>
  <Tab title="自动化 JSON 输出">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://kova.ai/install-cli.sh | bash -s -- --json --prefix /opt/openclaw
    ```
  </Tab>
  <Tab title="运行新手引导">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://kova.ai/install-cli.sh | bash -s -- --onboard
    ```
  </Tab>
</Tabs>
...
| `--version <ver>`      | Kova 版本或 dist-tag（默认：`latest`）                             |
...
| `OPENCLAW_VERSION=<ver>`                    | Kova 版本或 dist-tag                               |
...
<Tabs>
  ...
  <Tab title="Git 安装">
    ```powershell
    & ([scriptblock]::Create((iwr -useb https://kova.ai/install.ps1))) -InstallMethod git
    ```
  </Tab>
  <Tab title="通过 npm 安装 GitHub main">
    ```powershell
    & ([scriptblock]::Create((iwr -useb https://kova.ai/install.ps1))) -Tag main
    ```
  </Tab>
  <Tab title="自定义 git 目录">
    ```powershell
    & ([scriptblock]::Create((iwr -useb https://kova.ai/install.ps1))) -InstallMethod git -GitDir "C:\openclaw"
    ```
  </Tab>
  <Tab title="试运行">
    ```powershell
    & ([scriptblock]::Create((iwr -useb https://kova.ai/install.ps1))) -DryRun
    ```
  </Tab>
  <Tab title="调试跟踪">
    ```powershell
...
<Tabs>
  <Tab title="install.sh（非交互式 npm）">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://kova.ai/install.sh | bash -s -- --no-prompt --no-onboard
    ```
  </Tab>
...
<AccordionGroup>
  ...
  <Accordion title="sharp/libvips 问题">
    ...
    ```bash
    SHARP_IGNORE_GLOBAL_LIBVIPS=0 curl -fsSL --proto '=https' --tlsv1.2 https://kova.ai/install.sh | bash
    ```
