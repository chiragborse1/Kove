...
2. 在 `~/.kova/openclaw.json` 中配置 WhatsApp。
...
WhatsApp 需要真实手机号码进行验证。VoIP 和虚拟号码通常会被封锁。在 WhatsApp 上运行 Kova 有两种支持的方式：

### 专用号码（推荐）

为 Kova 使用**单独的手机号码**。最佳用户体验，清晰的路由，无自聊天怪异问题。理想设置：**备用/旧 Android 手机 + eSIM**。保持 Wi-Fi 和电源连接，通过二维码关联。

**WhatsApp Business：** 你可以在同一设备上使用不同号码的 WhatsApp Business。非常适合将个人 WhatsApp 分开——安装 WhatsApp Business 并在那里注册 Kova 号码。
...
快速备选方案：在**你自己的号码**上运行 Kova。给自己发消息（WhatsApp"给自己发消息"）进行测试，这样就不会打扰联系人。在设置和实验期间需要在主手机上阅读验证码。**必须启用自聊天模式。**
...
- 早期 Kova 版本支持 Twilio 的 WhatsApp Business 集成。
...
- 凭证存储在 `~/.kova/credentials/whatsapp/<accountId>/creds.json`。
- 备份副本在 `creds.json.bak`（损坏时恢复）。
- 旧版兼容性：较旧的安装将 Baileys 文件直接存储在 `~/.kova/credentials/` 中。
...
如果你在**个人 WhatsApp 号码**上运行 Kova，请启用 `channels.whatsapp.selfChatMode`（见上面的示例）。
...
**当我关联 WhatsApp 时，Kova 会给随机联系人发消息吗？**
不会。默认私信策略是**配对**，因此未知发送者只会收到配对码，他们的消息**不会被处理**。Kova 只会回复它收到的聊天，或你明确触发的发送（智能体/CLI）。
...
**多个人可以在一个 WhatsApp 号码上使用不同的 Kova 实例吗？**
...
- 最佳效果：OGG/Opus。Kova 将 `audio/ogg` 重写为 `audio/ogg; codecs=opus`。
