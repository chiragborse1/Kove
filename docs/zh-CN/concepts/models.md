...
- `agents.defaults.models` 是 Kova 可使用的模型允许列表/目录（以及别名）。
...
- 如果你省略提供商，Kova 会将输入视为别名或 **默认提供商** 的模型（仅在模型 ID 中没有 `/` 时有效）。
...
智能体目录下的 `models.json`（默认是 `~/.kova/agents/<agentId>/agent/models.json`）。默认会合并此文件，除非将 `models.mode` 设置为 `replace`。
...
标记持久化以源为准：Kova 会根据活动源配置快照（预解析）写入标记，而不是根据运行时已解析的 secret 值写入。
这适用于 Kova 重新生成 `models.json` 的所有情况，包括像 `openclaw agent` 这样的命令驱动路径。
