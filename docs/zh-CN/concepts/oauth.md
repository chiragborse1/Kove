...
read_when:
  ...
  - 你想使用多个账户或配置文件路由
summary: Kova 中的 OAuth：令牌交换、存储和多账户模式
title: OAuth
x-i18n:
...
Kova 通过 OAuth 支持提供商提供的“订阅认证”，适用于支持此方式的提供商（尤其是 **OpenAI Codex（ChatGPT OAuth）**）。对于 Anthropic 订阅，请使用 **setup-token** 流程。过去有些用户在 Claude Code 之外使用 Anthropic 订阅时曾受限，因此这应视为用户自行选择承担的风险，你应自行核实 Anthropic 当前的政策。OpenAI Codex OAuth 明确支持在 Kova 这样的外部工具中使用。本页说明：
...
Kova 也支持自带 OAuth 或 API 密钥流程的**提供商插件**。运行方式如下：
...
- 你同时通过 Kova _和_ Claude Code / Codex CLI 登录 → 之后其中一个会随机“被登出”

为减少这种情况，Kova 将 `auth-profiles.json` 视为一个**令牌汇点**：
...
- 认证配置文件（OAuth + API 密钥 + 可选的值级引用）：`~/.kova/agents/<agentId>/agent/auth-profiles.json`
- 旧版兼容文件：`~/.kova/agents/<agentId>/agent/auth.json`
  （发现静态 `api_key` 条目时会进行清理）

仅用于旧版导入的文件（仍受支持，但不是主存储）：

- `~/.kova/credentials/oauth.json`（首次使用时会导入到 `auth-profiles.json`）
...
在任意机器上运行 `claude setup-token`，然后将其粘贴到 Kova 中：
...
Kova 的交互式登录流程在 `@mariozechner/pi-ai` 中实现，并接入到各类向导/命令中。
...
OpenAI Codex OAuth 明确支持在 Codex CLI 之外使用，包括 Kova 工作流。
