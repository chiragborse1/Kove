---
read_when:
  - 理解 Kova 中 Pi SDK 集成设计时
  - 修改 Pi 的智能体会话生命周期、工具或提供商接线时
summary: Kova 内嵌 Pi 智能体集成与会话生命周期的架构
title: Pi 集成架构
x-i18n:
...
本文档介绍 Kova 如何与 [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) 及其同级软件包（`pi-ai`、`pi-agent-core`、`pi-tui`）集成，以提供其 AI 智能体能力。

## 概览

Kova 使用 pi SDK 将 AI 编码智能体嵌入到其消息 Gateway 网关架构中。Kova 不会将 pi 作为子进程启动，也不会使用 RPC 模式，而是通过 `createAgentSession()` 直接导入并实例化 pi 的 `AgentSession`。这种内嵌方式提供了：
...
| `pi-tui`          | 终端 UI 组件（用于 Kova 的本地 TUI 模式）                                              |
...
图像注入仅限当前提示：Kova 会从当前提示中加载图像引用，并仅通过 `images` 将其传入该轮。它不会重新扫描较早的历史轮次来重新注入图像负载。
...
2. **自定义替换**：Kova 用 `exec` / `process` 替换 bash，并为沙箱定制 read / edit / write
3. **Kova 工具**：消息、浏览器、画布、会话、cron、Gateway 网关 等
...
这样可以确保 Kova 的策略过滤、沙箱集成和扩展工具集在不同提供商之间保持一致。

## 系统提示词构建

系统提示词在 `buildAgentSystemPrompt()`（`system-prompt.ts`）中构建。它会组装完整提示词，包含工具、工具调用风格、安全护栏、Kova CLI 参考、Skills、文档、工作区、沙箱、消息、回复标签、语音、静默回复、心跳、运行时元数据等部分，并在启用时包含 Memory 和 Reactions，以及可选的上下文文件和额外系统提示词内容。为子智能体使用的最小提示词模式会对各部分进行裁剪。
...
Kova 通过 `guardSessionManager()` 对其进行包装，以保证工具结果安全。
...
Kova 维护一个凭证配置存储，为每个提供商保存多个 API 密钥：
...
Kova 还提供本地 TUI 模式，可直接使用 pi-tui 组件：
...
| 工具       | 默认编码工具            | 自定义 Kova 工具套件                                                                        |
| 系统提示词 | AGENTS.md + prompts     | 按渠道 / 上下文动态生成                                                                         |
| 会话存储   | `~/.pi/agent/sessions/` | `~/.kova/agents/<agentId>/sessions/`（或 `$OPENCLAW_STATE_DIR/agents/<agentId>/sessions/`） |
| 身份验证   | 单一凭证                | 支持轮换的多配置                                                                                |
