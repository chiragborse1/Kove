...
`openclaw doctor` 是 Kova 的修复 + 迁移工具。它修复过时的配置/状态，检查健康状况，并提供可操作的修复步骤。
...
- 解释找到了哪些遗留键。
- 显示它应用的迁移。
- 使用更新后的 schema 重写 `~/.kova/openclaw.json`。
...
- 会话存储 + 记录：
  - 从 `~/.kova/sessions/` 到 `~/.kova/agents/<agentId>/sessions/`
- 智能体目录：
  - 从 `~/.kova/agent/` 到 `~/.kova/agents/<agentId>/agent/`
- WhatsApp 认证状态（Baileys）：
  - 从遗留的 `~/.kova/credentials/*.json`（除 `oauth.json` 外）
  - 到 `~/.kova/credentials/whatsapp/<accountId>/...`（默认账户 id：`default`）
...
- **配置文件权限**：当 `~/.kova/openclaw.json` 对组/其他用户可读时发出警告，并提供收紧到 `600` 的选项。
...
Doctor 检测遗留的 Gateway 网关服务（launchd/systemd/schtasks），并提供删除它们并使用当前 Gateway 网关端口安装 Kova 服务的选项。它还可以扫描额外的类 Gateway 网关服务并打印清理提示。
配置文件命名的 Kova Gateway 网关服务被视为一等公民，不会被标记为"额外的"。
