---
description: Complete field-by-field reference for ~/.kova/openclaw.json
read_when:
  - 你需要精确到字段级别的配置语义或默认值
  - 你正在验证渠道、模型、Gateway 网关或工具配置块
summary: 每个 Kova 配置键、默认值和渠道设置的完整参考
title: 配置参考
x-i18n:
...
`~/.kova/openclaw.json` 中所有可用字段。若需面向任务的概览，请参见 [Configuration](/gateway/configuration)。

配置格式为 **JSON5**（允许注释和尾随逗号）。所有字段都是可选的——省略时，Kova 会使用安全默认值。
...
- 在重复解密失败后，Kova 还会尝试通过离开/重新加入语音会话来恢复语音接收。
...
- `commands.callbackUrl` 必须解析到 Kova Gateway 网关端点，并且 Mattermost 服务器可以访问它。
...
Kova 会启动 `imsg rpc`（通过 stdio 的 JSON-RPC）。不需要守护进程或端口。
...
- 如果你通过 `openclaw channels add`（或渠道新手引导）添加了一个非默认账户，而当前仍是单账户顶层渠道配置，Kova 会先将带账户作用域的顶层单账户值移动到 `channels.<channel>.accounts.default`，以便原始账户继续工作。
...
可选的仓库根目录，会显示在系统提示的 Runtime 行中。如果未设置，Kova 会从工作区向上遍历自动检测。
...
- `model.primary`：格式为 `provider/model`（例如 `anthropic/claude-opus-4-6`）。如果省略 provider，Kova 会假定为 `anthropic`（已弃用）。
...
- `timeoutSeconds`：Kova 中止前，单次压缩操作允许的最大秒数。默认：`900`。
...
- `identityData` / `certificateData` / `knownHostsData`：内联内容或 SecretRef，Kova 会在运行时将其物化为临时文件
...
- `none`：位于 `~/.kova/sandboxes` 下的按作用域划分的沙箱工作区
...
在 `remote` 模式下，在 Kova 外部对主机本地所做的编辑，不会在种子步骤后自动同步进沙箱。
...
noVNC 观察者访问默认使用 VNC 身份验证，Kova 会发出一个短期有效的 token URL（而不是在共享 URL 中暴露密码）。
...
对于 `type: "acp"` 条目，Kova 会按精确对话身份（`match.channel` + account + `match.peer.id`）解析，不使用上述 route 绑定层级顺序。
...
- **`parentForkMaxTokens`**：创建分叉线程会话时，父会话允许的最大 `totalTokens`（默认 `100000`）。
  - 如果父会话 `totalTokens` 高于该值，Kova 会启动一个新的线程会话，而不是继承父会话的记录历史。
...
- 当 `openai.baseUrl` 指向非 OpenAI 端点时，Kova 会将其视为兼容 OpenAI 的 TTS 服务器，并放宽模型/语音校验。
...
Kova 使用 pi-coding-agent 模型目录。可通过配置中的 `models.providers` 或 `~/.kova/agents/<agentId>/agent/models.json` 添加自定义提供商。
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`：可选的兼容性提示。对 `api: "openai-completions"` 且非空、非原生 `baseUrl`（主机不是 `api.openai.com`）的情况，Kova 会在运行时将其强制设为 `false`。空或省略的 `baseUrl` 会保留默认 OpenAI 行为。
...
- 从 `~/.kova/extensions`、`<workspace>/.openclaw/extensions` 以及 `plugins.load.paths` 加载。
- 设备发现同时接受原生 Kova 插件、兼容的 Codex bundle 和 Claude bundle，包括无 manifest 的 Claude 默认布局 bundle。
- **配置更改需要重启 Gateway 网关。**
...
- `plugins.entries.<id>.config`：插件定义的配置对象（如有可用原生 Kova 插件 schema，则会校验）。
- 已启用的 Claude bundle 插件也可以从 `settings.json` 提供嵌入式 Pi 默认值；Kova 会将其作为净化后的智能体设置应用，而不是作为原始 Kova 配置补丁。
...
在 `~/.kova/dns/` 下写入一个单播 DNS-SD 区域。对于跨网络设备发现，可与 DNS 服务器（推荐 CoreDNS）+ Tailscale split DNS 搭配使用。
...
- `.env` 文件：当前工作目录 `.env` + `~/.kova/.env`（两者都不会覆盖现有变量）。
...
- 旧版 OAuth 会从 `~/.kova/credentials/oauth.json` 导入。
...
- `cli.banner.taglineMode` 控制横幅标语样式：
  ...
  - `"default"`：固定的中性标语（`All your chats, one Kova.`）。
