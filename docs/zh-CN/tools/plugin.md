...
- 原生 **Kova 插件**（`openclaw.plugin.json` + 运行时模块），或
- 兼容的 **bundle**（`.codex-plugin/plugin.json` 或 `.claude-plugin/plugin.json`）

两者都会显示在 `openclaw plugins` 下，但只有原生 Kova 插件会在进程内执行运行时代码。

大多数情况下，当你想要某项核心 Kova 尚未内置的功能时，就会使用插件（或者你希望将可选功能保留在主安装之外）。
...
裸规格和 `@latest` 会停留在稳定通道。如果 npm 将其中任一解析为预发布版本，Kova 会停止并要求你通过预发布标签（例如 `@beta`/`@rc`）或精确的预发布版本显式选择加入。
...
1. **清单 + 发现**
   Kova 会从已配置路径、工作区根目录、全局扩展根目录以及随附扩展中查找候选插件。发现过程会先读取原生 `openclaw.plugin.json` 清单以及受支持的 bundle 清单。
2. **启用 + 验证**
   核心会决定某个已发现插件是启用、禁用、阻止，还是被选用于某个独占插槽（例如内存）。
3. **运行时加载**
   原生 Kova 插件通过 jiti 在进程内加载，并将功能注册到一个中央注册表中。兼容的 bundle 会被规范化为注册表记录，而不会导入运行时代码。
4. **表面消费**
   Kova 的其余部分会读取注册表，以公开工具、渠道、提供商设置、hooks、HTTP 路由、CLI 命令和服务。
...
这种拆分让 Kova 能够在完整运行时激活之前，就验证配置、解释缺失/被禁用的插件，并构建 UI/schema 提示。

## 兼容的 bundle

Kova 还识别两种兼容的外部 bundle 布局：
...
目前，Kova 将这些内容视为 **能力包**，而不是原生运行时插件：

- 当前支持：捆绑的 `skills`
- 当前支持：Claude `commands/` markdown 根目录，映射到常规 Kova 技能加载器
- 当前支持：Claude bundle `settings.json` 默认值，用于嵌入式 Pi 智能体设置（会清理 shell override 键）
- 当前支持：Cursor `.cursor/commands/*.md` 根目录，映射到常规 Kova 技能加载器
- 当前支持：使用 Kova hook-pack 布局的 Codex bundle hook 目录（`HOOK.md` + `handler.ts`/`handler.js`）
...
Bundle hook 支持仅限于常规 Kova hook 目录格式（在声明的 hook 根目录下使用 `HOOK.md` 加上 `handler.ts`/`handler.js`）。
供应商特定的 shell/JSON hook 运行时，包括 Claude `hooks.json`，当前仅会被检测，不会被直接执行。

## 执行模型

原生 Kova 插件与 Gateway 网关 **在同一进程内** 运行。它们不会进行沙箱隔离。已加载的原生插件与核心代码处于相同的进程级信任边界。

这意味着：

- 原生插件可以注册工具、网络处理器、hooks 和服务
- 原生插件中的 bug 可能导致 gateway 崩溃或不稳定
- 恶意原生插件等同于在 Kova 进程内部执行任意代码

兼容的 bundle 默认更安全，因为 Kova 目前将它们视为元数据/内容包。在当前版本中，这主要意味着捆绑的技能。
...
原生 Kova 插件是通过 jiti 在运行时加载的 **TypeScript 模块**。
...
原生 Kova 插件与 Gateway 网关 **在同一进程内** 运行，因此应将其视为受信任代码。
...
Kova 仍然负责通用的智能体循环、故障切换、转录处理和工具策略。这些 hooks 是 provider 特定行为的接缝，而无需整个自定义推理传输层。
...
对于 model/provider 插件，Kova 大致按以下顺序使用 hooks：
...
如果 provider 需要完全自定义的线协议或自定义请求执行器，那属于另一类扩展。这些 hooks 适用于仍运行在 Kova 常规推理循环上的 provider 行为。
...
- OpenRouter 使用 `catalog` 以及 `resolveDynamicModel` 和 `prepareDynamicModel`，因为该 provider 是透传型的，可能会在 Kova 静态目录更新之前暴露新的 model id。
...
编写插件时，请使用 SDK 子路径，而不是单体式 `getkova/plugin-sdk` 导入：

- `getkova/plugin-sdk/core` 用于通用插件 API、provider 身份验证类型和共享辅助工具。
- `getkova/plugin-sdk/compat` 用于比 `core` 需要更广泛共享运行时辅助工具的捆绑/内部插件代码。
- `getkova/plugin-sdk/telegram` 用于 Telegram 渠道插件。
- `getkova/plugin-sdk/discord` 用于 Discord 渠道插件。
- `getkova/plugin-sdk/slack` 用于 Slack 渠道插件。
- `getkova/plugin-sdk/signal` 用于 Signal 渠道插件。
- `getkova/plugin-sdk/imessage` 用于 iMessage 渠道插件。
- `getkova/plugin-sdk/whatsapp` 用于 WhatsApp 渠道插件。
- `getkova/plugin-sdk/line` 用于 LINE 渠道插件。
- `getkova/plugin-sdk/msteams` 用于捆绑的 Microsoft Teams 插件表面。
- 也提供捆绑扩展专用子路径：
  `getkova/plugin-sdk/acpx`、`getkova/plugin-sdk/bluebubbles`、
  `getkova/plugin-sdk/copilot-proxy`、`getkova/plugin-sdk/device-pair`、
  `getkova/plugin-sdk/diagnostics-otel`、`getkova/plugin-sdk/diffs`、
  `getkova/plugin-sdk/feishu`、`getkova/plugin-sdk/googlechat`、
  `getkova/plugin-sdk/irc`、`getkova/plugin-sdk/llm-task`、
  `getkova/plugin-sdk/lobster`、`getkova/plugin-sdk/matrix`、
  `getkova/plugin-sdk/mattermost`、`getkova/plugin-sdk/memory-core`、
  `getkova/plugin-sdk/memory-lancedb`、
  `getkova/plugin-sdk/minimax-portal-auth`、
  `getkova/plugin-sdk/nextcloud-talk`、`getkova/plugin-sdk/nostr`、
  `getkova/plugin-sdk/open-prose`、`getkova/plugin-sdk/phone-control`、
  `getkova/plugin-sdk/synology-chat`、
  `getkova/plugin-sdk/talk-voice`、`getkova/plugin-sdk/test-utils`、
  `getkova/plugin-sdk/thread-ownership`、`getkova/plugin-sdk/tlon`、
  `getkova/plugin-sdk/twitch`、`getkova/plugin-sdk/voice-call`、
  `getkova/plugin-sdk/zalo` 和 `getkova/plugin-sdk/zalouser`。
...
`catalog.order` 控制插件目录相对于 Kova 内置隐式 provider 的合并时机：
...
- 如果同时注册了 `catalog` 和 `discovery`，Kova 会使用 `catalog`

兼容性说明：

- `getkova/plugin-sdk` 仍支持现有外部插件。
...
- 如果 `plugins.allow` 为空且可发现非捆绑插件，Kova 会在启动时记录一条警告，其中包含 plugin id 和来源。
- 候选路径在被接受进入发现流程前会经过安全检查。当出现以下情况时，Kova 会阻止候选项：
...
每个原生 Kova 插件都必须在其根目录中包含一个 `openclaw.plugin.json` 文件。
...
当 Kova 需要为一个已禁用的渠道插件提供 setup 表面，或某个渠道插件虽然已启用但仍未配置时，它会加载 `setupEntry`，而不是完整插件入口。
...
Kova 还可以合并 **外部渠道目录**（例如 MPM registry 导出）。
将 JSON 文件放到以下任一位置：
...
如果插件导出 `id`，Kova 会使用它，但如果它与配置的 id 不匹配，则会发出警告。
...
- 兼容的 bundle 当前不公开原生 Kova 配置 schema。
...
Kova 会保留已禁用插件的配置，因此重新启用它们不会造成破坏。
...
openclaw plugins install <path>                 # 将本地文件/目录复制到 ~/.kova/extensions/<id>
...
如果更新之间存储的完整性元数据发生变化，Kova 会发出警告并要求确认（使用全局 `--yes` 可绕过提示）。
...
```ts
import { delegateCompactionToRuntime } from "getkova/plugin-sdk";
...
- 插件管理的 hooks 会在 `kova hooks list` 中显示为 `plugin:<id>`。
- 你不能通过 `kova hooks` 启用/禁用插件管理的 hooks；应改为启用/禁用插件本身。
...
- 禁用后，Kova 会阻止 `before_prompt_build`，并忽略旧版 `before_agent_start` 返回的提示词变更字段，同时保留旧版 `modelOverride` 和 `providerOverride`。
...
插件可以注册 **模型 providers**，从而让用户能够在 Kova 内完成 OAuth 或 API 密钥设置、在新手引导/模型选择器中显示 provider 设置，并参与隐式 provider 发现。
...
当 provider 具有多种身份验证方法时，向导可以显式指向其中一种方法，也可以让 Kova 为每种方法自动生成选择项。

在插件注册时，Kova 会验证 provider 向导元数据：
...
- `openclaw plugins install <npm-spec>` 会使用 `npm pack`，提取到 `~/.kova/extensions/<id>/`，并在配置中启用它。
