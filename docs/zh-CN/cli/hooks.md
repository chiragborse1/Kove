...
read_when:
  - 你想管理智能体钩子
  - 你想安装或更新钩子
summary: CLI 参考：`kova hooks`（智能体钩子）
title: hooks
x-i18n:
...
Details:
  ...
  Homepage: https://docs.kova.ai/automation/hooks#session-memory
  Events: command:new
...
通过将特定钩子添加到配置（`~/.kova/config.json`）来启用它。

**注意：** 由插件管理的钩子在 `kova hooks list` 中显示 `plugin:<id>`，
...
```bash
# 本地目录
kova hooks install ./my-hook-pack

# 本地压缩包
kova hooks install ./my-hook-pack.zip

# NPM 包
kova hooks install @openclaw/my-hook-pack

# 链接本地目录而不复制
kova hooks install -l ./my-hook-pack
```

## 更新钩子

```bash
kova hooks update <id>
kova hooks update --all
```
...
**输出：** `~/.kova/workspace/memory/YYYY-MM-DD-slug.md`
