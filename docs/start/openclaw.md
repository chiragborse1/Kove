---
summary: "End-to-end guide for running Kova as a personal assistant with safety cautions"
read_when:
...
Kova is a self-hosted gateway that connects WhatsApp, Telegram, Discord, iMessage, and more to AI agents. This guide covers the "personal assistant" setup: a dedicated WhatsApp number that behaves like your always-on AI assistant.
...
- Kova installed and onboarded — see [Getting Started](/start/getting-started) if you haven't done this yet
...
If you link your personal WhatsApp to Kova, every message to you becomes “agent input”. That’s rarely what you want.
...
3. Put a minimal config in `~/.kova/openclaw.json`:
...
Tip: treat this folder like Kova’s “memory” and make it a git repo (ideally private) so your `AGENTS.md` + memory files are backed up. If git is installed, brand-new workspaces are auto-initialized.
...
Kova defaults to a good assistant setup, but you’ll usually want to tune:
...
- Session files: `~/.kova/agents/<agentId>/sessions/{{SessionId}}.jsonl`
- Session metadata (token usage, last route, etc): `~/.kova/agents/<agentId>/sessions/sessions.json` (legacy: `~/.kova/sessions/sessions.json`)
...
Kova extracts these and sends them as media alongside the text.

Local-path behavior follows the same file-read trust model as the agent:

- If `tools.fs.workspaceOnly` is `true`, outbound `MEDIA:` local paths stay restricted to the Kova temp root, the media cache, agent workspace paths, and sandbox-generated files.
