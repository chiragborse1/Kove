...
- **Tailoring lives outside the repo:** `~/.kova/workspace` (workspace) + `~/.kova/openclaw.json` (config).
...
- **Config:** `~/.kova/openclaw.json` (JSON/JSON5-ish)
...
If you don’t have a global install yet, run it via `pnpm kova setup`.
...
- Keep `~/.kova/workspace` and `~/.kova/` as “your stuff”; don’t put personal prompts/config into the `openclaw` repo.
