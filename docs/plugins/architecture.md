...
read_when:
  - Building or debugging native Kova plugins
...
This page covers the internal architecture of the Kova plugin system.

## Public capability model

Capabilities are the public **native plugin** model inside Kova. Every
native Kova plugin registers against one or more capability types:
...
Kova classifies every loaded plugin into a shape based on its actual
...
Kova's plugin system has four layers:

1. **Manifest + discovery**
   Kova finds candidate plugins from configured paths, workspace roots,
...
3. **Runtime loading**
   Native Kova plugins are loaded in-process via jiti and register
...
4. **Surface consumption**
   The rest of Kova reads the registry to expose tools, channels, provider
...
That keeps plugin-owned CLI code inside the plugin while still letting Kova
...
That split lets Kova validate config, explain missing/disabled plugins, and
...
normal chat actions. Kova keeps one shared `message` tool in core, and
...
Kova treats a native plugin as the ownership boundary for a **company** or a
**feature**, not as a grab bag of unrelated integrations.

That means:

- a company plugin should usually own all of that company's Kova-facing
  surfaces
...
So if Kova adds a new domain such as video, the first question is not
...
A company plugin should feel cohesive from the outside. If Kova has shared
...
Kova already treats image/audio/video understanding as one shared
...
If Kova adds a new domain later, such as video generation, use the same
...
`KovaPluginApi`. That contract defines the supported registration points and
...
2. **contract tests**
   ...
   Kova can assert ownership explicitly. Today this is used for model
   providers, speech providers, web search providers, and bundled registration
   ownership.

The practical effect is that Kova knows, up front, which plugin owns which
...
- ad hoc runtime objects that are not part of `KovaPluginApi` or
  `api.runtime`
...
Native Kova plugins run **in-process** with the Gateway. They are not
...
Compatible bundles are safer by default because Kova currently treats them
...
Kova exports capabilities, not implementation convenience.
...
- OpenRouter uses `catalog` plus `resolveDynamicModel` and
  ...
  model ids before Kova's static catalog updates; it also uses
...
- If Kova adds a new capability such as video generation later, define the
...
Use SDK subpaths instead of the monolithic `getkova/plugin-sdk` import when
authoring plugins:

- `getkova/plugin-sdk/plugin-entry` for plugin registration primitives.
- `getkova/plugin-sdk/core` for the generic shared plugin-facing contract.
- Stable channel primitives such as `getkova/plugin-sdk/channel-setup`,
  `getkova/plugin-sdk/channel-pairing`,
  `getkova/plugin-sdk/channel-contract`,
  `getkova/plugin-sdk/channel-feedback`,
  `getkova/plugin-sdk/channel-inbound`,
  `getkova/plugin-sdk/channel-lifecycle`,
  `getkova/plugin-sdk/channel-reply-pipeline`,
  `getkova/plugin-sdk/command-auth`,
  `getkova/plugin-sdk/secret-input`, and
  `getkova/plugin-sdk/webhook-ingress` for shared setup/auth/reply/webhook
...
- Domain subpaths such as `getkova/plugin-sdk/channel-config-helpers`,
  `getkova/plugin-sdk/allow-from`,
  `getkova/plugin-sdk/channel-config-schema`,
  `getkova/plugin-sdk/channel-policy`,
  `getkova/plugin-sdk/approval-runtime`,
  `getkova/plugin-sdk/config-runtime`,
  `getkova/plugin-sdk/infra-runtime`,
  `getkova/plugin-sdk/agent-runtime`,
  `getkova/plugin-sdk/lazy-runtime`,
  `getkova/plugin-sdk/reply-history`,
  `getkova/plugin-sdk/routing`,
  `getkova/plugin-sdk/status-helpers`,
  `getkova/plugin-sdk/runtime-store`, and
  `getkova/plugin-sdk/directory-runtime` for shared runtime/config helpers.
...
- `getkova/plugin-sdk/channel-runtime` remains only as a compatibility shim.
...
- Bundled extension internals remain private. External plugins should use only
  `getkova/plugin-sdk/*` subpaths. Kova core/test code may use the repo
...
- Avoid the root `getkova/plugin-sdk` barrel for new code.
- Prefer the narrow stable primitives first. The newer setup/pairing/reply/
  ...
  Target parsing/matching belongs on `getkova/plugin-sdk/channel-targets`.
  Message action gates and reaction message-id helpers belong on
  `getkova/plugin-sdk/channel-actions`.
- Bundled extension-specific helper barrels are not stable by default. If a
  ...
  `getkova/plugin-sdk/<extension>`.
- New shared helper seams should be generic, not channel-branded. Shared target
  parsing belongs on `getkova/plugin-sdk/channel-targets`; channel-specific
...
```ts
import { delegateCompactionToRuntime } from "getkova/plugin-sdk/core";
