...
Kova can convert outbound replies into audio using ElevenLabs, Microsoft, or OpenAI.
It works anywhere Kova can send audio.
...
When `messages.tts.provider` is unset, Kova picks the first configured
...
- If `provider` is **unset**, Kova uses the first configured speech provider in registry auto-select order.
...
`~/.kova/settings/tts.json`, override with `OPENCLAW_TTS_PREFS` or
...
Discord note: `/tts` is a built-in Discord command, so Kova registers
