...
Kova routes replies **back to the channel where a message came from**. The
...
Kova infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, Kova still records inbound session metadata, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when Kova would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.kova/workspace-support" }],
  },
  bindings: [
...
- `~/.kova/agents/<agentId>/sessions/sessions.json`
