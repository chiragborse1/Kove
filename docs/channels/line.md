...
read_when:
  - You want to connect Kova to LINE
...
LINE connects to Kova via the LINE Messaging API. The plugin runs as a webhook
...
- LINE signature verification is body-dependent (HMAC over the raw body), so Kova applies strict pre-auth body limits and timeout before verification.
- Kova processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
