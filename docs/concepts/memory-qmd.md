...
alongside Kova. It combines BM25, vector search, and reranking in a single
...
- **Automatic fallback** -- if QMD is unavailable, Kova falls back to the
...
Kova creates a self-contained QMD home under
`~/.kova/agents/<agentId>/qmd/` and manages the sidecar lifecycle
automatically -- collections, updates, and embedding runs are handled for you.

## How the sidecar works

- Kova creates collections from your workspace memory files and any
...
- Searches use the configured `searchMode` (default: `search`; also supports
  `vsearch` and `query`). If a mode fails, Kova retries with `qmd query`.
- If QMD fails entirely, Kova falls back to the builtin SQLite engine.
...
collection under `~/.kova/agents/<id>/qmd/sessions/`.
...
When scope denies a search, Kova logs a warning with the derived channel and
...
**QMD not found?** Ensure the binary is on the gateway's `PATH`. If Kova
runs as a service, create a symlink:
`sudo ln -s ~/.bun/bin/qmd /usr/local/bin/qmd`.

**First search very slow?** QMD downloads GGUF models on first use. Pre-warm
with `qmd query "test"` using the same XDG dirs Kova uses.
...
Kova's builtin symlink rules. Keep temporary monorepo checkouts under
