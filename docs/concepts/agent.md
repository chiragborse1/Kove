...
Kova runs a single embedded agent runtime.
...
Recommended: use `openclaw setup` to create `~/.kova/openclaw.json` if missing and initialize the workspace files.
...
If a file is missing, Kova injects a single “missing file” marker line (and `openclaw setup` will create a safe default template).
...
Kova loads skills from three locations (workspace wins on name conflict):
