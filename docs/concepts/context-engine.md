...
read_when:
  - You want to understand how Kova assembles model context
...
A **context engine** controls how Kova builds model context for each run.
It decides which messages to include, how to summarize older history, and how
to manage context across subagent boundaries.

Kova ships with a built-in `legacy` engine. Plugins can register
...
cat ~/.kova/openclaw.json | jq '.plugins.slots.contextEngine'
```

### Installing a context engine plugin

Context engine plugins are installed like any other Kova plugin. Install
...
Every time Kova runs a model prompt, the context engine participates at
...
Kova currently calls one subagent lifecycle hook:
...
The `assemble` method can return a `systemPromptAddition` string. Kova
...
The built-in `legacy` engine preserves Kova's original behavior:
...
- `estimatedTokens` (required, `number`) — the engine's estimate of total
  tokens in the assembled context. Kova uses this for compaction threshold
...
- `true` — the engine owns compaction behavior. Kova disables Pi's built-in
...
`ownsCompaction: false` does **not** mean Kova automatically falls back to
...
- **Delegating mode** — set `ownsCompaction: false` and have `compact()` call
  `delegateCompactionToRuntime(...)` from `getkova/plugin-sdk/core` to use
  Kova's built-in compaction behavior.
...
Kova resolves when it needs a context engine.

## Relationship to compaction and memory

- **Compaction** is one responsibility of the context engine. The legacy engine
  delegates to Kova's built-in summarization. Plugin engines can implement
...
- Engine errors are logged and surfaced in diagnostics. If a plugin engine
  fails to register or the selected engine id cannot be resolved, Kova
