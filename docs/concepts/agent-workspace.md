...
This is separate from `~/.kova/`, which stores config, credentials, and
sessions.
...
inside a sandbox workspace under `~/.kova/sandboxes`, not your host workspace.
...
- Override in `~/.kova/openclaw.json`:
...
If any bootstrap file is missing, Kova injects a "missing file" marker into
...
- `~/.kova/openclaw.json` (config)
- `~/.kova/credentials/` (OAuth tokens, API keys)
- `~/.kova/agents/<agentId>/sessions/` (session transcripts + metadata)
- `~/.kova/skills/` (managed skills)
...
secret elsewhere (password manager, environment variables, or `~/.kova/`).
...
1. Clone the repo to the desired path (default `~/.kova/workspace`).
2. Set `agents.defaults.workspace` to that path in `~/.kova/openclaw.json`.
3. Run `openclaw setup --workspace <path>` to seed any missing files.
4. If you need sessions, copy `~/.kova/agents/<agentId>/sessions/` from the
