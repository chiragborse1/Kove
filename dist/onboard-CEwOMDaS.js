...
import { f as readConfigFileSnapshot } from "./io-DcGUiGNW.js";
import { s as isValidEnvSecretRefId } from "./types.secrets-mzCLKGg0.js";
...
import { c as normalizeGatewayTokenInput, d as randomToken, f as resolveControlUiLinks, g as waitForGatewayReachable, n as applyWizardMetadata, o as handleReset, r as ensureWorkspaceAndSessions, t as DEFAULT_WORKSPACE } from "./onboard-helpers-BPgzEhsc.js";
...
import { t as runSetupWizard } from "./setup-BOAZknZY.js";
import { a as resolveManifestProviderOnboardAuthFlags } from "./provider-auth-choices-DAWVxAzr.js";
import { i as resolveDeprecatedAuthChoiceReplacement, n as isDeprecatedAuthChoice, r as normalizeLegacyOnboardAuthChoice, t as formatDeprecatedNonInteractiveAuthChoiceError } from "./auth-choice-legacy-CWDsJJGa.js";
import { r as applyLocalSetupWorkspaceConfig } from "./onboard-config-B2ZQTX0H.js";
...
async function collectGatewayHealthFailureDiagnostics() {
	const diagnostics = {};
	try {
		const { resolveGatewayService } = await import("./service-DAVJpsU2.js");
	...
	try {
		const { readLastGatewayErrorLine } = await import("./diagnostics-BK2DUqtJ.js");
...
async function runNonInteractiveLocalSetup(params) {
	...
	if (authChoice !== "skip") {
		const { applyNonInteractiveAuthChoice } = await import("./auth-choice-B5tUUEtw.js");
	...
	if (opts.installDaemon) {
		const { installGatewayDaemonNonInteractive } = await import("./daemon-install-BrNOZEwr.js");
	...
	if (!opts.json) runtime.log(`Tip: run \`${formatCliCommand("kova configure --section web")}\` to store your Brave API key for web_search. Docs: https://docs.kova.ai/tools/web`);
}
//#endregion
...
async function runNonInteractiveRemoteSetup(params) {
	...
	if (opts.json) writeRuntimeJson(runtime, payload);
		...
		runtime.log(`Tip: run \`${formatCliCommand("kova configure --section web")}\` to store your Brave API key for web_search. Docs: https://docs.kova.ai/tools/web`);
	}
}
//#endregion
...
async function runNonInteractiveSetup(opts, runtime = defaultRuntime) {
	...
	if (snapshot.exists && !snapshot.valid) {
		runtime.error(`Config invalid. Run \`${formatCliCommand("kova doctor")}\` to repair it, then re-run setup.`);
		runtime.exit(1);
		return;
	}
...
async function setupWizardCommand(opts, runtime = defaultRuntime) {
	...
	if (normalizedOpts.nonInteractive && normalizedOpts.acceptRisk !== true) {
		runtime.error([
			...
			"Read: https://docs.kova.ai/security",
	...
	if (process.platform === "win32") runtime.log([
		"Windows detected - Kova runs great on WSL2!",
		"Native Windows might be trickier.",
		"Quick setup: wsl --install (one command, one reboot)",
		"Guide: https://docs.kova.ai/windows"
	].join("\n"));
