import { _ as resolveOAuthDir, s as resolveConfigPath, y as resolveStateDir } from "./paths-CG2veZVr.js";
...
import { c as loadConfig, i as createConfigIO } from "./io-DcGUiGNW.js";
import "./config-Dl06sDj4.js";
import { a as createIcaclsResetCommand, o as formatIcaclsResetCommand } from "./audit-fs-DxxNYgY4.js";
...
import { o as getSecurityAuditCommandSecretTargetIds } from "./command-secret-targets-BlPJW81G.js";
...
import { t as runSecurityAudit } from "./audit-CdFYkCHn.js";
...
function registerSecurityCli(program) {
	program.command("security").description("Audit local config and state for common security foot-guns").addHelpText("after", () => `\n${theme.heading("Examples:")}\n${formatHelpExamples([
		...
		["kova security audit --deep --password <password>", "Use explicit password for deep probe."],
		...
		const lines = [];
		lines.push(heading("Kova security audit"));
		lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
		lines.push(muted(`Run deeper: ${formatCliCommand("kova security audit --deep")}`));
		for (const diagnostic of secretDiagnostics) lines.push(muted(`[secrets] ${diagnostic}`));
		if (opts.fix) {
			lines.push(muted(`Fix: ${formatCliCommand("kova security audit --fix")}`));
