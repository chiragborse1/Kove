import { T as resolveRequiredHomeDir, y as resolveStateDir } from "./paths-CG2veZVr.js";
...
import { d as shouldPreferNativeJiti } from "./bundled-plugin-metadata-BoHf6EFf.js";
...
const MATRIX_LEGACY_CRYPTO_INSPECTOR_UNAVAILABLE_MESSAGE = "Legacy Matrix encrypted state was detected, but the Matrix plugin helper is unavailable. Install or repair @openclaw/matrix so Kova can inspect the old rust crypto store before upgrading.";
...
async function autoPrepareLegacyMatrixCrypto(params) {
	...
	for (const plan of detection.plans) {
		...
		if (!plan.deviceId) {
			warnings.push(`Legacy Matrix encrypted state detected at ${plan.legacyCryptoPath}, but no device ID was found for account "${plan.accountId}". Kova will continue, but old encrypted history cannot be recovered automatically.`);
			continue;
		}
		let summary;
		try {
