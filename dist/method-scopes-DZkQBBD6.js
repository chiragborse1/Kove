...
import { n as logError, t as logDebug } from "./logger-DiTvWLNg.js";
import { n as VERSION } from "./version-By7KE3vz.js";
import { r as getActivePluginRegistry } from "./runtime-D0EER0KC.js";
import { n as ENV_SECRET_REF_ID_RE } from "./types.secrets-mzCLKGg0.js";
import { n as FILE_SECRET_REF_ID_PATTERN, r as SECRET_PROVIDER_ALIAS_PATTERN, t as EXEC_SECRET_REF_ID_JSON_SCHEMA_PATTERN } from "./ref-contract-CC70pc8C.js";
import { a as isSecureWebSocketUrl, r as isLoopbackHost } from "./net-CTrWm98z.js";
import { _ as GATEWAY_CLIENT_NAMES, g as GATEWAY_CLIENT_MODES, h as GATEWAY_CLIENT_IDS } from "./message-channel-CAdct431.js";
import { a as signDevicePayload, i as publicKeyRawBase64UrlFromPem, n as loadOrCreateDeviceIdentity } from "./device-identity-CNuUiVKA.js";
import { n as normalizeDeviceAuthScopes, t as normalizeDeviceAuthRole } from "./device-auth-D2AfWmtU.js";
import { t as safeParseJsonWithSchema } from "./zod-parse-rmymmP8h.js";
import { t as rawDataToString } from "./ws-yXNLGnMX.js";
import { t as normalizeDeviceMetadataForAuth } from "./device-metadata-normalization-D8aMjqVM.js";
import { t as INPUT_PROVENANCE_KIND_VALUES } from "./input-provenance-DtYt5HRh.js";
...
//#endregion
//#region src/gateway/protocol/schema/api-keys.ts
const ApiKeyProviderIdSchema = Type.Union([
...
const ApiKeyProviderStatusSchema = Type.Object({
...
const ApiKeysProvidersGetParamsSchema = Type.Object({}, { additionalProperties: false });
Type.Object({
	providers: Type.Array(ApiKeyProviderStatusSchema),
	currentModel: NonEmptyString,
	configHash: Type.Union([Type.String(), Type.Null()])
}, { additionalProperties: false });
const ApiKeysProviderSetParamsSchema = Type.Object({
	provider: ApiKeyProviderIdSchema,
	apiKey: Type.Optional(Type.String()),
	baseHash: Type.Optional(NonEmptyString)
}, { additionalProperties: false });
const ApiKeysProviderTestParamsSchema = Type.Object({
	provider: ApiKeyProviderIdSchema,
	apiKey: Type.Optional(Type.String()),
	model: Type.Optional(NonEmptyString)
}, { additionalProperties: false });
...
const ApiKeysActiveModelSetParamsSchema = Type.Object({
	provider: ApiKeyProviderIdSchema,
	model: NonEmptyString,
	baseHash: Type.Optional(NonEmptyString)
}, { additionalProperties: false });
const ApiKeysOpenRouterGetParamsSchema = Type.Object({}, { additionalProperties: false });
Type.Object({
...
const ApiKeysOpenRouterSetParamsSchema = Type.Object({
	apiKey: Type.Optional(Type.String()),
	model: Type.Optional(NonEmptyString),
	baseHash: Type.Optional(NonEmptyString)
}, { additionalProperties: false });
const ApiKeysOpenRouterTestParamsSchema = Type.Object({
	apiKey: Type.Optional(Type.String()),
	model: Type.Optional(NonEmptyString)
}, { additionalProperties: false });
Type.Object({
...
const ChannelsLoginParamsSchema = Type.Object({
	channel: NonEmptyString,
	force: Type.Optional(Type.Boolean()),
	timeoutMs: Type.Optional(Type.Integer({ minimum: 0 })),
...
const ChannelsLoginWaitParamsSchema = Type.Object({
	channel: NonEmptyString,
	timeoutMs: Type.Optional(Type.Integer({ minimum: 0 })),
	accountId: Type.Optional(Type.String())
}, { additionalProperties: false });
...
//#endregion
//#region src/gateway/protocol/schema/pairing.ts
const PairingListParamsSchema = Type.Object({
	channel: NonEmptyString,
	accountId: Type.Optional(NonEmptyString)
}, { additionalProperties: false });
const PairingApproveParamsSchema = Type.Object({
	channel: NonEmptyString,
	code: NonEmptyString,
	accountId: Type.Optional(NonEmptyString)
}, { additionalProperties: false });
const PairingRejectParamsSchema = Type.Object({
	channel: NonEmptyString,
	code: NonEmptyString,
	accountId: Type.Optional(NonEmptyString)
}, { additionalProperties: false });
...
Type.Object({
	...
	auth: Type.Optional(Type.Object({
		...
		sharedToken: Type.Optional(NonEmptyString),
		role: Type.Optional(NonEmptyString),
		scopes: Type.Optional(Type.Array(NonEmptyString)),
...
const validateApiKeysProvidersGetParams = ajv.compile(ApiKeysProvidersGetParamsSchema);
const validateApiKeysProviderSetParams = ajv.compile(ApiKeysProviderSetParamsSchema);
const validateApiKeysProviderTestParams = ajv.compile(ApiKeysProviderTestParamsSchema);
const validateApiKeysActiveModelSetParams = ajv.compile(ApiKeysActiveModelSetParamsSchema);
const validateApiKeysOpenRouterGetParams = ajv.compile(ApiKeysOpenRouterGetParamsSchema);
const validateApiKeysOpenRouterSetParams = ajv.compile(ApiKeysOpenRouterSetParamsSchema);
const validateApiKeysOpenRouterTestParams = ajv.compile(ApiKeysOpenRouterTestParamsSchema);
...
const validateChannelsLoginParams = ajv.compile(ChannelsLoginParamsSchema);
const validateChannelsLoginWaitParams = ajv.compile(ChannelsLoginWaitParamsSchema);
...
const validatePairingListParams = ajv.compile(PairingListParamsSchema);
const validatePairingApproveParams = ajv.compile(PairingApproveParamsSchema);
const validatePairingRejectParams = ajv.compile(PairingRejectParamsSchema);
...
//#endregion
export { validateDevicePairListParams as $, validateSkillsUpdateParams as $t, validateChannelsLoginWaitParams as A, normalizeFingerprint as An, validatePluginApprovalResolveParams as At, validateConfigSchemaLookupParams as B, validateSessionsDeleteParams as Bt, validateApiKeysOpenRouterGetParams as C, parseSessionLabel as Cn, validateNodePendingDrainParams as Ct, validateApiKeysProviderTestParams as D, getPreauthHandshakeTimeoutMsFromEnv as Dn, validatePairingListParams as Dt, validateApiKeysProviderSetParams as E, resolveDeviceAuthConnectErrorDetailCode as En, validatePairingApproveParams as Et, validateChatInjectParams as F, validateSecretsResolveResult as Ft, validateCronAddParams as G, validateSessionsPreviewParams as Gt, validateConfigSchemaParams as H, validateSessionsMessagesSubscribeParams as Ht, validateChatSendParams as I, validateSendParams as It, validateCronRunParams as J, validateSessionsSendParams as Jt, validateCronListParams as K, validateSessionsResetParams as Kt, validateConfigApplyParams as L, validateSessionsAbortParams as Lt, validateChannelsStatusParams as M, validatePushTestParams as Mt, validateChatAbortParams as N, validateRequestFrame as Nt, validateApiKeysProvidersGetParams as O, buildDeviceAuthPayload as On, validatePairingRejectParams as Ot, validateChatHistoryParams as P, validateSecretsResolveParams as Pt, validateDevicePairApproveParams as Q, validateSkillsStatusParams as Qt, validateConfigGetParams as R, validateSessionsCompactParams as Rt, validateApiKeysActiveModelSetParams as S, errorShape as Sn, validateNodePendingAckParams as St, validateApiKeysOpenRouterTestParams as T, resolveAuthConnectErrorDetailCode as Tn, validateNodeRenameParams as Tt, validateConfigSetParams as U, validateSessionsMessagesUnsubscribeParams as Ut, validateConfigSchemaLookupResult as V, validateSessionsListParams as Vt, validateConnectParams as W, validateSessionsPatchParams as Wt, validateCronStatusParams as X, validateSkillsBinsParams as Xt, validateCronRunsParams as Y, validateSessionsUsageParams as Yt, validateCronUpdateParams as Z, validateSkillsInstallParams as Zt, validateAgentsFilesGetParams as _, approvalDecisionLabel as _n, validateNodePairApproveParams as _t, READ_SCOPE as a, validateUpdateRunParams as an, validateExecApprovalResolveParams as at, validateAgentsListParams as b, buildPluginApprovalResolvedMessage as bn, validateNodePairRequestParams as bt, isNodeRoleMethod as c, validateWebLoginWaitParams as cn, validateExecApprovalsNodeSetParams as ct, formatValidationErrors as d, validateWizardStartParams as dn, validateModelsListParams as dt, validateTalkConfigParams as en, validateDevicePairRejectParams as et, validateAgentIdentityParams as f, validateWizardStatusParams as fn, validateNodeDescribeParams as ft, validateAgentsDeleteParams as g, PLUGIN_APPROVAL_TITLE_MAX_LENGTH as gn, validateNodeListParams as gt, validateAgentsCreateParams as h, PLUGIN_APPROVAL_DESCRIPTION_MAX_LENGTH as hn, validateNodeInvokeResultParams as ht, PAIRING_SCOPE as i, validateToolsEffectiveParams as in, validateExecApprovalRequestParams as it, validateChannelsLogoutParams as j, validatePollParams as jt, validateChannelsLoginParams as k, buildDeviceAuthPayloadV3 as kn, validatePluginApprovalRequestParams as kt, resolveLeastPrivilegeOperatorScopesForMethod as l, validateWizardCancelParams as ln, validateExecApprovalsSetParams as lt, validateAgentWaitParams as m, MAX_PLUGIN_APPROVAL_TIMEOUT_MS as mn, validateNodeInvokeParams as mt, APPROVALS_SCOPE as n, validateTalkSpeakParams as nn, validateDeviceTokenRevokeParams as nt, WRITE_SCOPE as o, validateWakeParams as on, validateExecApprovalsGetParams as ot, validateAgentParams as p, DEFAULT_PLUGIN_APPROVAL_TIMEOUT_MS as pn, validateNodeEventParams as pt, validateCronRemoveParams as q, validateSessionsResolveParams as qt, CLI_DEFAULT_OPERATOR_SCOPES as r, validateToolsCatalogParams as rn, validateDeviceTokenRotateParams as rt, authorizeOperatorScopesForMethod as s, validateWebLoginStartParams as sn, validateExecApprovalsNodeGetParams as st, ADMIN_SCOPE as t, validateTalkModeParams as tn, validateDevicePairRemoveParams as tt, GatewayClient as u, validateWizardNextParams as un, validateLogsTailParams as ut, validateAgentsFilesListParams as v, buildPluginApprovalExpiredMessage as vn, validateNodePairListParams as vt, validateApiKeysOpenRouterSetParams as w, ConnectErrorDetailCodes as wn, validateNodePendingEnqueueParams as wt, validateAgentsUpdateParams as x, ErrorCodes as xn, validateNodePairVerifyParams as xt, validateAgentsFilesSetParams as y, buildPluginApprovalRequestMessage as yn, validateNodePairRejectParams as yt, validateConfigPatchParams as z, validateSessionsCreateParams as zt };
