import { r as DEFAULT_PROVIDER } from "./defaults-OPjrzs9a.js";
...
import { i as buildModelAliasIndex, v as resolveModelRefFromString } from "./model-selection-CdS674HX.js";
...
import { n as resolvePluginWebSearchConfig } from "./legacy-web-search-BztdBZ6G.js";
...
function shouldIncludeStartupQrLoginChannelPlugin(params) {
	if (params.config.web?.enabled === false) return false;
...
function resolveGatewayStartupPluginIds(params) {
	...
	return manifestRegistry.plugins.filter((plugin) => {
		...
		if (plugin.channels.length > 0) return shouldIncludeStartupQrLoginChannelPlugin({
			plugin,
			config: params.config,
			pluginsConfig
		});
