...
function isUnhandledRejectionHandled(reason) {
	for (const handler of handlers) try {
		...
		console.error("[kova] Unhandled rejection handler failed:", err instanceof Error ? err.stack ?? err.message : err);
	}
	return false;
}
function installUnhandledRejectionHandler() {
	process.on("unhandledRejection", (reason, _promise) => {
		...
		if (isAbortError(reason)) {
			console.warn("[kova] Suppressed AbortError:", formatUncaughtError(reason));
			return;
		}
		if (isFatalError(reason)) {
			console.error("[kova] FATAL unhandled rejection:", formatUncaughtError(reason));
			process.exit(1);
			return;
		}
		if (isConfigError(reason)) {
			console.error("[kova] CONFIGURATION ERROR - requires fix:", formatUncaughtError(reason));
			process.exit(1);
			return;
		}
		if (isTransientUnhandledRejectionError(reason)) {
			console.warn("[kova] Non-fatal unhandled rejection (continuing):", formatUncaughtError(reason));
			return;
		}
		console.error("[kova] Unhandled promise rejection:", formatUncaughtError(reason));
		process.exit(1);
	});
}
//#endregion
