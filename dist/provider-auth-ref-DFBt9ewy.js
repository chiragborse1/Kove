...
import { a as formatExecSecretRefIdValidationMessage, l as resolveDefaultSecretProviderAlias, o as isValidExecSecretRefId, s as isValidFileSecretRefId } from "./ref-contract-CC70pc8C.js";
...
function loadSecretResolve() {
	secretResolvePromise ??= import("./resolve-RPYbXk5o.js");
...
async function promptEnvSecretRefForSetup(params) {
	...
	await params.prompter.note(params.copy?.envValidatedMessage?.(envVar) ?? `Validated environment variable ${envVar}. Kova will store a reference, not the key value.`, "Reference validated");
	return {
		ref,
		resolvedValue
	};
...
async function promptProviderSecretRefForSetup(params) {
	...
	try {
		...
		await params.prompter.note(params.copy?.providerValidatedMessage?.(selectedProvider, id, providerEntry.source) ?? `Validated ${providerEntry.source} reference ${selectedProvider}:${id}. Kova will store a reference, not the key value.`, "Reference validated");
		return {
			ref,
			resolvedValue
		};
