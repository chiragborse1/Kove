...
import { n as getNodeDaemonStatusSummary, t as getDaemonStatusSummary } from "./status.daemon-DEoO7_UY.js";
import "./status.summary-CXUHTZWE.js";
...
function loadProviderUsage() {
	providerUsagePromise ??= import("./provider-usage-AQn7E_8z.js");
	return providerUsagePromise;
}
function loadSecurityAuditModule() {
	securityAuditModulePromise ??= import("./audit.runtime-BovUflpB.js");
	return securityAuditModulePromise;
}
function loadGatewayCallModule() {
	gatewayCallModulePromise ??= import("./call-DEzb0ME1.js");
	return gatewayCallModulePromise;
}
function loadStatusScanModule() {
	statusScanModulePromise ??= import("./status.scan-2Z9X3aJ7.js");
	return statusScanModulePromise;
}
function loadStatusScanFastJsonModule() {
	statusScanFastJsonModulePromise ??= import("./status.scan.fast-json-CxRgGSho.js");
	return statusScanFastJsonModulePromise;
}
function loadStatusAllModule() {
	statusAllModulePromise ??= import("./status-all-CcvizvWC.js");
	return statusAllModulePromise;
}
function loadStatusCommandTextRuntime() {
	statusCommandTextRuntimePromise ??= import("./status.command.text-runtime-1CC4hvgd.js");
	return statusCommandTextRuntimePromise;
}
function loadStatusNodeModeModule() {
	statusNodeModeModulePromise ??= import("./status.node-mode-Df0C3v3K.js");
...
async function statusCommand(opts, runtime) {
	...
	runtime.log(theme.heading("Kova status"));
	runtime.log("");
	...
	if (summary.taskAudit.errors > 0) {
		runtime.log("");
		runtime.log(theme.muted(`Task maintenance: ${formatCliCommand("kova tasks maintenance --apply")}`));
	}
	...
	if (pairingRecovery) {
		...
		if (pairingRecovery.requestId) runtime.log(theme.muted(`Recovery: ${formatCliCommand(`kova devices approve ${pairingRecovery.requestId}`)}`));
		runtime.log(theme.muted(`Fallback: ${formatCliCommand("kova devices approve --latest")}`));
		runtime.log(theme.muted(`Inspect: ${formatCliCommand("kova devices list")}`));
	}
	runtime.log("");
	...
	runtime.log(theme.muted(`Full report: ${formatCliCommand("kova security audit")}`));
	runtime.log(theme.muted(`Deep probe: ${formatCliCommand("kova security audit --deep")}`));
	runtime.log("");
	...
	runtime.log("");
	runtime.log("FAQ: https://docs.kova.ai/faq");
	runtime.log("Troubleshooting: https://docs.kova.ai/troubleshooting");
	runtime.log("");
	...
	runtime.log(`  Need to share?      ${formatCliCommand("kova status --all")}`);
	runtime.log(`  Need to debug live? ${formatCliCommand("kova logs --follow")}`);
	if (nodeOnlyGateway) runtime.log(`  Need node service?  ${formatCliCommand("kova node status")}`);
	else if (gatewayReachable) runtime.log(`  Need to test channels? ${formatCliCommand("kova status --deep")}`);
	else runtime.log(`  Fix reachability first: ${formatCliCommand("kova gateway probe")}`);
}
//#endregion
