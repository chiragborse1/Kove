import { d as resolveGatewayPort, s as resolveConfigPath, y as resolveStateDir } from "./paths-CG2veZVr.js";
import { n as defaultRuntime } from "./runtime-kS8e4c6-.js";
import { o as getResolvedLoggerSettings } from "./logger-Cnt8m3Fg.js";
import { a as toKovaDisplayLogPath, r as ensureKovaTmpSymlinkSync } from "./tmp-openclaw-dir-FwzW84ZI.js";
import { o as displayKovaConfigPath, s as displayKovaHomeStatePath, w as sleep } from "./utils-CQHEfYrT.js";
...
import { f as resolveGatewaySystemdServiceName, l as resolveGatewayLaunchAgentLabel } from "./constants-Ce1e_DDq.js";
import { s as trimToUndefined } from "./credential-planner-B0vdUPPY.js";
import { n as isGatewaySecretRefUnavailableError } from "./credentials-D-Qn12vP.js";
import { i as parseStrictPositiveInteger } from "./parse-finite-number-R3QtRTnY.js";
import { d as loadGatewayTlsRuntime } from "./call-Cmc_PDdk.js";
import { r as isWSLEnv } from "./wsl-DCvWupN8.js";
import { _ as inspectBestEffortPrimaryTailnetIPv4, f as resolveControlUiLinks, y as resolveBestEffortGatewayBindHostForDisplay } from "./onboard-helpers-BPgzEhsc.js";
import { c as resolveGatewayLogPaths } from "./launchd-Cx-dD96U.js";
import { r as resolveGatewayService } from "./service-C_gX92o7.js";
import { a as inspectPortUsage, o as classifyPortListener, s as formatPortDiagnostics } from "./ports-BBrhj0lW.js";
import { t as killProcessTree } from "./kill-tree-DyFhPOYG.js";
import { p as classifySystemdUnavailableDetail } from "./systemd-BJqyInLQ.js";
import { a as filterDaemonEnv, c as pickProbeHostForBind, d as resolveRuntimeStatusColor, f as safeDaemonEnv, o as normalizeListenerAddress, s as parsePortFromArgs, t as createCliStatusTextStyles, u as renderRuntimeHints } from "./shared-BXizBDOo.js";
import { d as resolveDaemonContainerContext, i as auditGatewayServiceConfig, n as renderSystemdUnavailableHints, t as isSystemdUnavailableDetail, u as formatRuntimeStatus } from "./systemd-hints-C9EXVN-A.js";
import "./logging-B4Zjxkhm.js";
import { i as probeGateway } from "./probe-CFpxFdGB.js";
import { t as formatConfigIssueLine } from "./issue-format-BI5rIoW9.js";
...
import { i as resolveGatewayProbeAuthWithSecretInputs } from "./probe-auth-e8gknAQ6.js";
...
async function probeGatewayStatus(opts) {
	try {
		const result = await withProgress({
			...
			if (opts.requireRpc) {
				const { callGateway } = await import("./call-DEzb0ME1.js");
				await callGateway({
					url: opts.url,
					token: opts.token,
			...
			const { probeGateway } = await import("./probe-DKqVnTNJ.js");
...
function printDaemonStatus(status, opts) {
	...
	const displayStatePath = (input) => displayKovaHomeStatePath(input);
	...
	try {
		...
		defaultRuntime.log(`${label("File logs:")} ${infoText(toKovaDisplayLogPath(displayStatePath(logFile)))}`);
	} catch {}
	...
	if (service.command?.sourcePath) defaultRuntime.log(`${label("Service file:")} ${infoText(displayStatePath(service.command.sourcePath))}`);
	if (service.command?.workingDirectory) defaultRuntime.log(`${label("Working dir:")} ${infoText(displayStatePath(service.command.workingDirectory))}`);
	...
	if (service.configAudit?.issues.length) {
		...
		defaultRuntime.error(warnText(`Recommendation: run "${formatCliCommand("kova doctor")}" (or "${formatCliCommand("kova doctor --repair")}").`));
	}
	if (status.config) {
		...
		if (status.config.mismatch) {
			...
			defaultRuntime.error(errorText(`Fix: rerun \`${formatCliCommand("kova gateway install --force")}\` from the same --profile / OPENCLAW_STATE_DIR you expect.`));
		}
		spacer();
	}
	if (status.gateway) {
	...
	if (status.health && status.health.staleGatewayPids.length > 0 && service.runtime?.status === "running" && typeof service.runtime.pid === "number") {
		...
		defaultRuntime.error(errorText(`Fix: run ${formatCliCommand("kova gateway restart")} and re-check with ${formatCliCommand("kova gateway status --deep")}.`));
		spacer();
	}
	...
	if (service.runtime?.cachedLabel) {
		...
		defaultRuntime.error(errorText(`Then reinstall: ${formatCliCommand("kova gateway install")}`));
		spacer();
	}
	...
	defaultRuntime.log(`${label("Troubleshooting:")} https://docs.kova.ai/troubleshooting`);
}
//#endregion
