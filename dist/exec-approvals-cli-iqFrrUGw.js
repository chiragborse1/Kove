...
import { O as saveExecApprovals, v as readExecApprovalsSnapshot } from "./exec-approvals-C-ADQo5x.js";
import { n as formatTimeAgo } from "./format-relative-Bo03qcyi.js";
import { n as callGatewayFromCli } from "./gateway-rpc-D-Uiu9WJ.js";
import { n as renderTable, t as getTerminalTableWidth } from "./table-DN8e5QIM.js";
import { t as collectExecPolicyScopeSnapshots } from "./exec-approvals-effective-_T16CoX4.js";
import { t as describeUnknownError } from "./shared-BWotu43d.js";
import { a as resolveNodeId, r as nodesCallOpts } from "./rpc-CSmlZAUC.js";
...
function registerExecApprovalsCli(program) {
	...
	const allowlist = approvals.command("allowlist").description("Edit the per-agent allowlist").addHelpText("after", () => `\n${theme.heading("Examples:")}\n${formatExample("kova approvals allowlist add \"~/Projects/**/bin/rg\"", "Allowlist a local binary pattern for the main agent.")}\n${formatExample("kova approvals allowlist add --agent main --node <id|name|ip> \"/usr/bin/uptime\"", "Allowlist on a specific node/agent.")}\n${formatExample("kova approvals allowlist add --agent \"*\" \"/usr/bin/uname\"", "Allowlist for all agents (wildcard).")}\n${formatExample("kova approvals allowlist remove \"~/Projects/**/bin/rg\"", "Remove an allowlist pattern.")}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/approvals", "docs.kova.ai/cli/approvals")}\n`);
