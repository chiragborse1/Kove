...
import { b as resolveUserPath, c as displayPath } from "./utils-CQHEfYrT.js";
import { f as readConfigFileSnapshot, x as writeConfigFile } from "./io-DcGUiGNW.js";
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const { resolvePluginProviders, resolveProviderPluginChoice } = await import("./provider-auth-choice.runtime-Ct_9ueGN.js");
...
async function requireRiskAcknowledgement(params) {
	...
	await params.prompter.note([
		...
		"",
		"Kova is your personal AI team and still in beta. Expect sharp edges.",
		...
		"",
		"Must read: https://docs.kova.ai/gateway/security"
	].join("\n"), "Security");
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-B2V8HuBp.js");
	...
	if (snapshot.exists && !snapshot.valid) {
		...
		if (snapshot.issues.length > 0) await prompter.note([
			...
			"",
			"Docs: https://docs.kova.ai/gateway/configuration"
		].join("\n"), "Config issues");
		await prompter.outro(`Config invalid. Run \`${formatCliCommand("kova doctor")}\` to repair it, then re-run setup.`);
		runtime.exit(1);
		return;
	}
	...
	const quickstartHint = `Configure details later via ${formatCliCommand("kova configure")}.`;
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-BHO_RlGM.js");
		const { logConfigUpdated } = await import("./logging-FRo4w-ze.js");
	...
	const workspaceDir = resolveUserPath((opts.workspace ?? (flow === "quickstart" ? baseConfig.agents?.defaults?.workspace ?? onboardHelpers.DEFAULT_WORKSPACE : await prompter.text({
		...
		initialValue: displayPath(baseConfig.agents?.defaults?.workspace ?? onboardHelpers.DEFAULT_WORKSPACE)
	}))).trim() || onboardHelpers.DEFAULT_WORKSPACE);
	const { applyLocalSetupWorkspaceConfig } = await import("./onboard-config-pc3vFHoC.js");
	let nextConfig = applyLocalSetupWorkspaceConfig(baseConfig, workspaceDir);
	const { ensureAuthProfileStore } = await import("./agents/auth-profiles.runtime.js");
	const { promptAuthChoiceGrouped } = await import("./auth-choice-prompt-CWb6MeYq.js");
	const { promptCustomApiConfig } = await import("./onboard-custom-C83aF3o9.js");
	const { applyAuthChoice, resolvePreferredProviderForAuthChoice, warnIfModelConfigLooksOff } = await import("./auth-choice-4KbVluVk.js");
	const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-DsvEk8zX.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-QxZpkZJ6.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { listChannelPlugins } = await import("./plugins-Lwpz1W4y.js");
		const { setupChannels } = await import("./onboard-channels-C9PrWFli.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-BseSI0e8.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-DXu0a0oZ.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	const { setupInternalHooks } = await import("./onboard-hooks-DJTW0OIG.js");
	...
	await writeConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-Dtvu01co.js");
