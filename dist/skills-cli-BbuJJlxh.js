...
import { i as updateSkillsFromClawHub, n as readTrackedClawHubSkillSlugs, r as searchSkillsFromClawHub, t as installSkillFromClawHub } from "./skills-clawhub-3J6V5QYR.js";
//#region src/cli/skills-cli.format.ts
function appendClawHubHint(output, json) {
	if (json) return output;
	return `${output}\n\nTip: use \`kova skills search\`, \`kova skills install\`, and \`kova skills update\` for ClawHub-backed skills.`;
}
...
function formatSkillsList(report, opts) {
	...
	if (skills.length === 0) return appendClawHubHint(opts.eligible ? `No eligible skills found. Run \`${formatCliCommand("kova skills list")}\` to see all skills.` : "No skills found.", opts.json);
...
function formatSkillInfo(report, skillName, opts) {
	...
	if (!skill) {
		...
		return appendClawHubHint(`Skill "${skillName}" not found. Run \`${formatCliCommand("kova skills list")}\` to see available skills.`, opts.json);
	}
	...
	if (skill.primaryEnv && skill.missing.env.includes(skill.primaryEnv)) {
		...
		lines.push(`  Save via CLI: ${formatCliCommand(`kova config set skills.entries.${safeSkillKey}.apiKey YOUR_KEY`)}`);
		lines.push(`  Stored in: ${theme.muted("$OPENCLAW_CONFIG_PATH")} ${theme.muted("(default: ~/.kova/kova.json)")}`);
	}
	return appendClawHubHint(lines.join("\n"), opts.json);
}
...
async function loadSkillsStatusReport() {
	...
	const { buildWorkspaceSkillStatus } = await import("./skills-status-Yb4_We-D.js");
