...
import { a as resolveHookEntries } from "./config-BQOoidiu.js";
import { a as buildPluginStatusReport } from "./status-C60d7mDK.js";
import { n as renderTable, t as getTerminalTableWidth } from "./table-DN8e5QIM.js";
import { t as loadWorkspaceHookEntries } from "./workspace-DRMxLrJE.js";
import { t as buildWorkspaceHookStatus } from "./hooks-status-CKJWKGCp.js";
import { n as runPluginInstallCommand, t as runPluginUpdateCommand } from "./plugins-update-command-DfBYLt0R.js";
...
function formatHooksList(report, opts) {
	...
	if (hooks.length === 0) return opts.eligible ? `No eligible hooks found. Run \`${formatCliCommand("kova hooks list")}\` to see all hooks.` : "No hooks found.";
...
function formatHookInfo(report, hookName, opts) {
	...
	if (!hook) {
		...
		return `Hook "${hookName}" not found. Run \`${formatCliCommand("kova hooks list")}\` to see available hooks.`;
	}
	if (opts.json) return JSON.stringify({
		...hook,
...
function registerHooksCli(program) {
	...
	hooks.command("install").description("Deprecated: install a hook pack via `kova plugins install`").argument("<path-or-spec>", "Path to a hook pack or npm package spec").option("-l, --link", "Link a local path instead of copying", false).option("--pin", "Record npm installs as exact resolved <name>@<version>", false).action(async (raw, opts) => {
		defaultRuntime.log(theme.warn("`kova hooks install` is deprecated; use `kova plugins install`."));
		await runPluginInstallCommand({
			raw,
			opts
		});
	});
	hooks.command("update").description("Deprecated: update hook packs via `kova plugins update`").argument("[id]", "Hook pack id (omit with --all)").option("--all", "Update all tracked hooks", false).option("--dry-run", "Show what would change without writing", false).action(async (id, opts) => {
		defaultRuntime.log(theme.warn("`kova hooks update` is deprecated; use `kova plugins update`."));
