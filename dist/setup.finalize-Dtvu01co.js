...
import { b as resolveUserPath, g as pathExists } from "./utils-CQHEfYrT.js";
import { r as DEFAULT_BOOTSTRAP_FILENAME } from "./workspace-DBgoItUM.js";
...
import { n as openUrl, t as detectBrowserOpenSupport } from "./browser-open-DE2jOBq5.js";
import { f as resolveControlUiLinks, g as waitForGatewayReachable, i as formatControlUiSshHint, u as probeGatewayReachable } from "./onboard-helpers-BPgzEhsc.js";
import { r as resolveGatewayService, t as describeGatewayServiceRestart } from "./service-C_gX92o7.js";
import { i as isSystemdUserServiceAvailable } from "./systemd-BJqyInLQ.js";
import { t as listConfiguredWebSearchProviders } from "./runtime-PkX7T3dM.js";
...
import { t as runTui } from "./tui-iTFWnPrr.js";
...
async function finalizeSetupWizard(options) {
	...
	if (process.platform === "linux" && systemdAvailable) {
		const { ensureSystemdUserLingerInteractive } = await import("./systemd-linger-DDQYhqWY.js");
	...
	if (!opts.skipHealth) {
		...
		if (gatewayProbe.ok) try {
			...
			await prompter.note([
				"Docs:",
				"https://docs.kova.ai/gateway/health",
				"https://docs.kova.ai/gateway/troubleshooting"
			].join("\n"), "Health check help");
		}
	...
	await prompter.note([
		...
		gatewayStatusLine,
		"Docs: https://docs.kova.ai/web/control-ui"
	].filter(Boolean).join("\n"), "Control UI");
	...
	if (!opts.skipUi && gatewayProbe.ok) {
		...
		await prompter.note([
			...
			"Stored in: $OPENCLAW_CONFIG_PATH (default: ~/.kova/kova.json) under gateway.auth.token, or in OPENCLAW_GATEWAY_TOKEN.",
		...
		if (hatchChoice === "tui") {
			...
			await prompter.note([
				...
				controlUiOpened ? "Opened in your browser. Keep that tab to control Kova." : "Copy/paste this URL in a browser on this machine to control Kova.",
				controlUiOpenHint
			].filter(Boolean).join("\n"), "Dashboard ready");
		} else await prompter.note(`When you're ready: ${formatCliCommand("kova dashboard --no-open")}`, "Later");
	} else if (opts.skipUi) await prompter.note("Skipping Control UI/TUI prompts.", "Control UI");
	await prompter.note(["Back up your agent workspace.", "Docs: https://docs.kova.ai/concepts/agent-workspace"].join("\n"), "Workspace backup");
	await prompter.note("Running agents on your computer is risky — harden your setup: https://docs.kova.ai/security", "Security");
	...
	const { describeCodexNativeWebSearch } = await import("./codex-native-web-search-DuJ4AQW-.js");
	...
	if (webSearchProvider) {
		const { resolveExistingKey, hasExistingKey, hasKeyInEnv } = await import("./onboard-search-BseSI0e8.js");
		...
		if (!entry) await prompter.note([
			...
			"",
			"Docs: https://docs.kova.ai/tools/web"
		].join("\n"), "Web search");
			...
			`  ${formatCliCommand("kova configure --section web")}`,
			"",
			`Get your key at: ${entry?.signupUrl ?? "https://docs.kova.ai/tools/web"}`,
			"Docs: https://docs.kova.ai/tools/web"
		].join("\n"), "Web search");
		else await prompter.note([
		...
		const { hasExistingKey, hasKeyInEnv } = await import("./onboard-search-BseSI0e8.js");
		const legacyDetected = configuredSearchProviders.find((e) => hasExistingKey(nextConfig, e.id) || hasKeyInEnv(e));
		if (legacyDetected) await prompter.note([`Web search is available via ${legacyDetected.label} (auto-detected).`, "Docs: https://docs.kova.ai/tools/web"].join("\n"), "Web search");
	...
	await prompter.note("What now: https://kova.ai/showcase (\"What People Are Building\").", "What now");
	await prompter.outro(controlUiOpened ? "Onboarding complete. Dashboard opened; keep that tab to control Kova." : "Onboarding complete. Use the dashboard link above to control Kova.");
	return { launchedTui };
}
//#endregion
