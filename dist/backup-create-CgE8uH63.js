...
import { C as shortenHomePath, S as shortenHomeInString, b as resolveUserPath, g as pathExists, v as resolveHomeDir } from "./utils-CQHEfYrT.js";
...
async function resolveBackupPlanFromDisk(params = {}) {
	...
	if (includeWorkspace && configSnapshot.exists && !configSnapshot.valid) throw new Error(`Config invalid at ${shortenHomePath(configSnapshot.path)}. Kova cannot reliably discover custom workspaces for backup. Fix the config or rerun with --no-include-workspace for a partial backup.`);
...
async function createBackupArchive(opts = {}) {
	...
	const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-backup-"));
