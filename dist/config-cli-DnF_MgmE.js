import { t as CONFIG_PATH } from "./paths-CG2veZVr.js";
...
import { n as info, o as success, t as danger } from "./globals-CxHdnmm2.js";
import { F as validateConfigObjectRaw, N as collectUnsupportedSecretRefPolicyIssues, f as readConfigFileSnapshot } from "./io-DcGUiGNW.js";
import { d as resolveSecretInputRef, i as coerceSecretRef, s as isValidEnvSecretRefId } from "./types.secrets-mzCLKGg0.js";
import { a as formatExecSecretRefIdValidationMessage, c as isValidSecretProviderAlias, d as validateExecSecretRefId, o as isValidExecSecretRefId, s as isValidFileSecretRefId, u as secretRefKey } from "./ref-contract-CC70pc8C.js";
import { C as SecretProviderSchema } from "./zod-schema.core-1c4_V7ob.js";
import { r as replaceConfigFile } from "./config-Dl06sDj4.js";
import { a as resolveSecretRefValue } from "./resolve-CC1QIKuE.js";
import { t as formatCliCommand } from "./command-format-Wh1wnuz1.js";
import { a as normalizeConfigIssues, n as formatConfigIssueLines } from "./issue-format-BI5rIoW9.js";
import { c as resolveConfigSecretTargetByPath, n as discoverConfigSecretTargets } from "./target-registry-CNWSMfby.js";
import { i as redactConfigObject, n as readBestEffortRuntimeConfigSchema } from "./runtime-schema-OcAQNFK9.js";
import { n as setCommandJsonMode } from "./json-mode-BZshh-q5.js";
...
const CONFIG_SET_DESCRIPTION = [
	...
	"Examples:",
	formatCliCommand("kova config set gateway.port 19001 --strict-json"),
	formatCliCommand("kova config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN"),
	formatCliCommand("kova config set secrets.providers.vault --provider-source file --provider-path /etc/openclaw/secrets.json --provider-mode json"),
	formatCliCommand("kova config set --batch-file ./config-set.batch.json --dry-run")
].join("\n");
...
async function runConfigValidate(opts = {}) {
	const runtime = opts.runtime ?? defaultRuntime;
	let outputPath = CONFIG_PATH ?? "kova.json";
	try {
		const snapshot = await readConfigFileSnapshot();
...
function registerConfigCli(program) {
	const cmd = program.command("config").description("Non-interactive config helpers (get/set/unset/file/schema/validate). Run without subcommand for guided setup.").addHelpText("after", () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.kova.ai/cli/config")}\n`).option("--section <section>", "Configuration sections for guided setup (repeatable). Use with no subcommand.", (value, previous) => [...previous, value], []).action(async (opts) => {
		const { configureCommandFromSectionsArg } = await import("./configure-G-A1JrKF.js");
	...
	cmd.command("schema").description("Print the JSON schema for kova.json").action(async () => {
