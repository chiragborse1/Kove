import { S as resolveHomeRelativePath, _ as resolveOAuthDir, x as resolveEffectiveHomeDir, y as resolveStateDir } from "./paths-CG2veZVr.js";
...
function rewriteKovaDisplayPath(input) {
	if (!input) return input;
	return input.replace(/(\$OPENCLAW_HOME|~)([\\/])\.openclaw(?=$|[\\/])/g, "$1$2.kova").replace(/((?:\$OPENCLAW_HOME|~)(?:[\\/])\.kova(?:[\\/][^\\/\s]+)*)([\\/])openclaw\.json\b/g, (_match, prefix, sep) => `${prefix}${sep}kova.json`);
}
function shortenHomePathRaw(input) {
	if (!input) return input;
...
function displayKovaHomeStatePath(input) {
	return shortenHomePathRaw(input).replace(/(\$OPENCLAW_HOME|~)([\\/])\.openclaw(?=$|[\\/])/g, "$1$2.kova");
}
function displayKovaConfigPath(input) {
	return rewriteKovaDisplayPath(shortenHomePathRaw(input));
}
...
//#endregion
export { shortenHomePath as C, truncateUtf16Safe as D, toWhatsappJid as E, isPlainObject as O, shortenHomeInString as S, sliceUtf16Safe as T, resolveConfigDir as _, clampNumber as a, resolveUserPath as b, displayPath as c, escapeRegExp as d, isRecord as f, pathExists as g, normalizeE164 as h, clampInt as i, formatTerminalLink as k, displayString as l, jidToE164 as m, assertWebChannel as n, displayKovaConfigPath as o, isSelfChatMode as p, clamp as r, displayKovaHomeStatePath as s, CONFIG_DIR as t, ensureDir as u, resolveHomeDir as v, sleep as w, safeParseJson as x, resolveJidToE164 as y };
