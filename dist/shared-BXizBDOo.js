...
import { c as buildPlatformRuntimeLogHints, d as resolveDaemonContainerContext, l as buildPlatformServiceStartHints } from "./systemd-hints-C9EXVN-A.js";
...
function classifyDaemonHintText(text) {
	if (text.includes("kova gateway install") || text.startsWith("Service not installed. Run:")) return "install";
...
function renderGatewayServiceStartHints(env = process.env) {
	...
	const hints = buildPlatformServiceStartHints({
		...
		startCommand: formatCliCommand("kova gateway", env),
