...
import { a as fetchClawHubPackageVersion, d as satisfiesPluginApiRange, i as fetchClawHubPackageDetail, l as resolveLatestVersionFromPackage, n as downloadClawHubPackageArchive, s as parseClawHubPluginSpec, t as ClawHubRequestError, u as satisfiesGatewayMinimum } from "./clawhub-BCircDr8.js";
import { n as installPluginFromArchive } from "./install-DZ-oUtox.js";
...
function validateClawHubPluginPackage(params) {
	...
	if (compatibility?.pluginApiRange && !satisfiesPluginApiRange(runtimeVersion, compatibility.pluginApiRange)) return buildClawHubInstallFailure(`Plugin "${pkg.name}" requires plugin API ${compatibility.pluginApiRange}, but this Kova runtime exposes ${runtimeVersion}.`, CLAWHUB_INSTALL_ERROR_CODE.INCOMPATIBLE_PLUGIN_API);
	if (compatibility?.minGatewayVersion && !satisfiesGatewayMinimum(runtimeVersion, compatibility.minGatewayVersion)) return buildClawHubInstallFailure(`Plugin "${pkg.name}" requires Kova >=${compatibility.minGatewayVersion}, but this host is ${runtimeVersion}.`, CLAWHUB_INSTALL_ERROR_CODE.INCOMPATIBLE_GATEWAY);
	return null;
}
