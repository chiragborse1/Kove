...
import { a as READ_SCOPE, n as APPROVALS_SCOPE, o as WRITE_SCOPE, u as GatewayClient } from "./method-scopes-DZkQBBD6.js";
...
function registerMcpCli(program) {
	const mcp = program.command("mcp").description("Manage Kova MCP config and channel bridge");
	mcp.command("serve").description("Expose Kova channels over MCP stdio").option("--url <url>", "Gateway WebSocket URL (defaults to gateway.remote.url when configured)").option("--token <token>", "Gateway token (if required)").option("--token-file <path>", "Read gateway token from file").option("--password <password>", "Gateway password (if required)").option("--password-file <path>", "Read gateway password from file").option("--claude-channel-mode <mode>", "Claude channel notification mode: auto, on, or off", "auto").option("-v, --verbose", "Verbose logging to stderr", false).action(async (opts) => {
		try {
