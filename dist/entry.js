...
import { d as isRootHelpInvocation, f as isRootVersionInvocation, l as hasHelpOrVersion } from "./argv-CrD4Uaj1.js";
import { a as parseCliContainerArgs, n as applyCliProfileEnv, o as resolveCliContainerTarget, r as parseCliProfileArgs, t as normalizeWindowsArgv } from "./windows-argv-D5Kqyb7I.js";
import { i as ensureDefaultStateDirReady } from "./paths-CG2veZVr.js";
...
import { t as installProcessWarningFilter } from "./warning-filter-VHnyk8-S.js";
...
if (!isMainModule({
	...
	const { installGaxiosFetchCompat } = await import("./gaxios-fetch-compat-CNAeTwrV.js");
	...
	function ensureCliRespawnReady() {
		...
		child.once("error", (error) => {
			console.error("[kova] Failed to respawn CLI:", error instanceof Error ? error.stack ?? error.message : error);
			process$1.exit(1);
		});
		return true;
	}
	function tryHandleRootVersionFastPath(argv) {
		...
		Promise.all([import("./version-BpbKeRgM.js"), import("./git-commit-C75BZKPf.js")]).then(([{ VERSION }, { resolveCommitHash }]) => {
			const commit = resolveCommitHash({ moduleUrl: import.meta.url });
			console.log(commit ? `Kova ${VERSION} (${commit})` : `Kova ${VERSION}`);
			process$1.exit(0);
		}).catch((error) => {
			console.error("[kova] Failed to resolve version:", error instanceof Error ? error.stack ?? error.message : error);
			process$1.exitCode = 1;
		});
		return true;
	}
...
function tryHandleRootHelpFastPath(argv, deps = {}) {
	...
	const handleError = deps.onError ?? ((error) => {
		console.error("[kova] Failed to display help:", error instanceof Error ? error.stack ?? error.message : error);
		process$1.exitCode = 1;
	});
	...
	import("./root-help-CPEnrg2x.js").then(({ outputRootHelp }) => {
...
function runMainOrRootHelp(argv) {
	...
	import("./run-main-Dha460xp.js").then(({ runCli }) => runCli(argv)).catch((error) => {
		console.error("[kova] Failed to start CLI:", error instanceof Error ? error.stack ?? error.message : error);
		process$1.exitCode = 1;
	});
}
//#endregion
