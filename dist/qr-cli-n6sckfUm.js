...
import { a as hasConfiguredSecretInput, d as resolveSecretInputRef, l as normalizeSecretInputString } from "./types.secrets-mzCLKGg0.js";
...
import { r as resolveRequiredConfiguredSecretRefInputString } from "./resolve-configured-secret-input-string-CuZ3K2Az.js";
import { t as resolveCommandSecretRefsViaGateway } from "./command-secret-gateway-DxQmZTJq.js";
import { i as getQrRemoteCommandSecretTargetIds } from "./command-secret-targets-BlPJW81G.js";
import { n as resolveGatewayBindUrl, t as resolveTailnetHostWithRunner } from "./tailscale-status-CeVy_fjW.js";
import { a as PAIRING_SETUP_BOOTSTRAP_PROFILE, n as issueDeviceBootstrapToken } from "./device-bootstrap-C0Mil0m6.js";
	...
	program.command("qr").description("Generate an iOS pairing QR code and setup code").addHelpText("after", () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/qr", "docs.kova.ai/cli/qr")}\n`).option("--remote", "Use gateway.remote.url and gateway.remote token/password (ignores device-pair publicUrl)", false).option("--url <url>", "Override gateway URL used in the setup payload").option("--public-url <url>", "Override gateway public URL used in the setup payload").option("--token <token>", "Override gateway token for setup payload").option("--password <password>", "Override gateway password for setup payload").option("--setup-code-only", "Print only the setup code", false).option("--no-ascii", "Skip ASCII QR rendering").option("--json", "Output JSON", false).action(async (opts) => {
		try {
			...
			const lines = [
				...
				"Scan this with the Kova iOS app (Onboarding -> Scan QR).",
				""
			];
			if (opts.ascii !== false) {
			...
			lines.push(`${theme.muted("Setup code:")} ${setupCode}`, `${theme.muted("Gateway:")} ${resolved.payload.url}`, `${theme.muted("Auth:")} ${resolved.authLabel}`, `${theme.muted("Source:")} ${resolved.urlSource}`, "", "Approve after scan with:", `  ${theme.command("kova devices list")}`, `  ${theme.command("kova devices approve <requestId>")}`);
