import { d as resolveGatewayPort, n as DEFAULT_GATEWAY_PORT } from "./paths-CG2veZVr.js";
...
import { H as asRuntimeConfig, V as asResolvedSourceConfig, f as readConfigFileSnapshot } from "./io-DcGUiGNW.js";
import { i as nodeVersionSatisfiesEngine } from "./runtime-guard-DMJNv0NA.js";
import { r as replaceConfigFile } from "./config-Dl06sDj4.js";
import { n as resolveCliName } from "./cli-name-BMCbfW7p.js";
import { t as formatCliCommand } from "./command-format-Wh1wnuz1.js";
import { f as resolveGatewaySystemdServiceName, l as resolveGatewayLaunchAgentLabel, p as resolveGatewayWindowsTaskName } from "./constants-Ce1e_DDq.js";
import { n as stylePromptMessage } from "./prompt-style-738lx6X3.js";
import { r as resolveGatewayService } from "./service-C_gX92o7.js";
import { n as quoteCmdScriptArg } from "./cmd-argv-CSluFaKs.js";
import { r as runDaemonRestart, s as runDaemonInstall } from "./daemon-cli-UoBLYf6Z.js";
import { n as formatConfigIssueLines } from "./issue-format-BI5rIoW9.js";
import { a as terminateStaleGatewayPids, i as renderRestartDiagnostics, s as waitForGatewayHealthyRestart } from "./status-BPfK1jML.js";
import { r as formatDurationPrecise } from "./format-duration-BwgwzyDX.js";
import { t as formatHelpExamples } from "./help-format-B6p6tNX6.js";
import { r as installCompletion } from "./completion-cli-Dnh08Fj2.js";
import { n as renderTable, t as getTerminalTableWidth } from "./table-DN8e5QIM.js";
import { c as resolveEffectiveUpdateChannel, i as formatUpdateChannelLabel, l as resolveUpdateChannelDisplay, r as channelToNpmTag, s as normalizeUpdateChannel } from "./update-channels-CSuYrAWA.js";
import { n as compareSemverStrings, o as resolveNpmChannelTag, r as fetchNpmPackageTargetStatus, t as checkUpdateStatus } from "./update-check-DOpmXLpi.js";
import { n as runGatewayUpdate } from "./server-startup-matrix-migration-CtCLXswv.js";
import { d as resolveGlobalPackageRoot, i as createGlobalInstallEnv, l as resolveExpectedInstalledVersionFromSpec, m as readPackageVersion, n as cleanupGlobalRenameDirs, p as readPackageName, r as collectInstalledGlobalPackageErrors, s as globalInstallArgs, t as canResolveRegistryVersionForPackageTarget, u as resolveGlobalInstallSpec } from "./update-global-MbGSNuuu.js";
import { n as updateNpmInstalledPlugins, t as syncPluginsForUpdateChannel } from "./update-DWifhaH7.js";
import { a as isGitCheckout, c as resolveGitInstallDir, d as resolveTargetVersion, f as resolveUpdateRoot, i as isEmptyDir, l as resolveGlobalManager, m as tryWriteCompletionCache, n as createGlobalCommandRunner, o as normalizeTag, p as runUpdateStep, r as ensureGitCheckout, s as parseTimeoutMsOrExit, t as DEFAULT_PACKAGE_NAME, u as resolveNodeRunner } from "./shared-_GgQGsUa.js";
import { n as doctorCommand, t as selectStyled } from "./prompt-select-styled-B8c0q5ED.js";
import { r as ensureCompletionCacheExists, t as checkShellCompletionStatus } from "./doctor-completion-Da5WPQgr.js";
...
async function updateStatusCommand(opts) {
	...
	defaultRuntime.log(theme.heading("Kova update status"));
	defaultRuntime.log("");
...
const UPDATE_QUIPS = [
	...
	"Kova just upgraded. Sharper tools, cleaner runs.",
	"Update done! Check the changelog or just trust me, it's good.",
	"Rebuilt from the chaos of npm. Stronger now.",
	...
	"Upgrade complete. Please don't judge the reboot hair.",
...
async function tryInstallShellCompletion(opts) {
	...
	if (!status.profileInstalled) {
		...
		if (isCancel(shouldInstall) || !shouldInstall) {
			if (!opts.skipPrompt) defaultRuntime.log(theme.muted(`Skipped. Run \`${formatCliCommand("kova completion --install")}\` later to enable.`));
			return;
		}
...
async function maybeRestartService(params) {
		...
		try {
			...
			if (!params.opts.json && restartInitiated) {
				...
				if (health.healthy) defaultRuntime.log(theme.success("Daemon restart completed."));
					...
					defaultRuntime.log(theme.muted(`Run \`${formatCliCommand("kova gateway status --deep")}\` for details.`));
				}
				defaultRuntime.log("");
			}
		} catch (err) {
			if (!params.opts.json) {
				...
				defaultRuntime.log(theme.muted(`You may need to restart the service manually: ${formatCliCommand("kova gateway restart")}`));
			}
		}
		return;
	}
	if (!params.opts.json) {
		defaultRuntime.log("");
		if (params.result.mode === "npm" || params.result.mode === "pnpm") defaultRuntime.log(theme.muted(`Tip: Run \`${formatCliCommand("kova doctor")}\`, then \`${formatCliCommand("kova gateway restart")}\` to apply updates to a running gateway.`));
		else defaultRuntime.log(theme.muted(`Tip: Run \`${formatCliCommand("kova gateway restart")}\` to apply updates to a running gateway.`));
	}
}
async function updateCommand(opts) {
	...
	if (result.status === "skipped") {
		...
		if (result.reason === "not-git-install") {
			defaultRuntime.log(theme.warn(`Skipped: this Kova install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${formatCliCommand("kova doctor")}\` and \`${formatCliCommand("kova gateway restart")}\`.`));
			defaultRuntime.log(theme.muted("Examples: `npm install -g getkova@latest` or `pnpm add -g getkova@latest`"));
		}
		defaultRuntime.exit(0);
		return;
	}
...
async function updateWizardCommand(opts = {}) {
	if (!process.stdin.isTTY) {
		defaultRuntime.error("Update wizard requires a TTY. Use `kova update --channel <stable|beta|dev>` instead.");
		defaultRuntime.exit(1);
		return;
	}
	const timeoutMs = parseTimeoutMsOrExit(opts.timeout);
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.kova.ai/cli/update")}`;
	}).action(async (opts) => {
		try {
			await updateCommand({
	...
	])}\n\n${theme.heading("Notes:")}\n${theme.muted("- Shows current update channel (stable/beta/dev) and source")}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.kova.ai/cli/update")}`).action(async (opts, command) => {
		try {
