...
IMPORTANT — No `gh` CLI dependency. This skill uses curl + the GitHub REST API exclusively. The GH_TOKEN env var is already injected by Kova. Pass it as a Bearer token in all API calls:
...
- If curl returns an HTTP 401 or 403 → stop and tell the user:
  > "GitHub authentication failed. Please check your apiKey in the Kova dashboard or in the active Kova config path (`$OPENCLAW_CONFIG_PATH`, default `~/.kova/openclaw.json`) under `skills.entries.gh-issues`."
