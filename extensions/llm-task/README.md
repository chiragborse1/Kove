...
`openclaw.invoke --each`) without adding new Kova code per workflow.
...
This extension depends on Kova internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Kova extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.kova/extensions` as a standalone plugin directory.
